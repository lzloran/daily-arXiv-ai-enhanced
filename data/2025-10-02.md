...
Motivation: 现有的机器学习遗忘方法通常依赖辅助语言模型、保留数据集或商业AI服务，这在实际应用中不切实际且可能引入额外的隐私风险。

Method: 通过识别序列中的目标令牌（用于优化遗忘目标）和非目标令牌（用于保持模型效用），直接在令牌级别进行优化。

Result: 在多个基准数据集上，DTO的遗忘质量比最新基线方法提高了16.8倍，同时保持了可比的模型效用水平。
...
<details>
  <summary>Details</summary>
Motivation: 过程性活动助手在日常生活和专业场景中有广泛应用潜力，但相关系统开发仍未被充分探索

Method: 使用多媒体返回工具进行训练免费的交错多模态推理，具备灵活工具选择的代理特性

Result: 在ProMQA-Assembly数据集上提升了视觉语言模型的性能，特别是GPT-5和MiMo-VL模型

Conclusion: 该框架促进了视频和多模态任务的图像思维范式，推动了过程性活动助手的发展
...
Method: 通过精心设计的合成流水线生成任务，结合人工验证，评估代理在公共网络和私有公司知识库中识别支持事实的能力。

Result: 发布了涵盖10个领域的15个深度研究任务，评估了多种开源和闭源模型及策略，揭示了各自的优势和弱点。

Conclusion: DRBench为推进企业深度研究提供了关键路径，展示了评估AI代理处理复杂企业任务的有效性。
...
TL;DR: 开发PrimeX数据集，包含858名美国居民的民意调查数据、意见解释和世界观评估，用于研究如何利用个人信念系统改进语言模型的对齐能力。


<details>
  <summary>Details</summary>
Motivation: 随着语言模型的广泛应用，需要更好地将个体用户特征表示给模型，探索能否利用个人信念系统来改进模型的对齐能力。

Method: 构建PrimeX数据集，包含858名美国居民的公开意见调查数据，并收集两个额外的信念信息来源：受访者对其意见的书面解释，以及用于评估受访者世界观的原初世界信念调查。

Result: 通过广泛的数据分析，证明了信念解释和世界观信息对于个性化语言模型的价值，展示了这些额外信念信息对NLP和心理学研究社区的双重益处。

Conclusion: PrimeX数据集中的额外信念信息能够为NLP和心理学研究社区带来益处，为后续研究开辟了新的途径。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM开发将任务解决和偏好对齐分开处理，但在实际应用中，正确解决问题不足够，需要匹配用户偏好，特别是在冷启动或隐私限制的场景下。

Method: 使用基于心理学的角色构建稀疏偏好，创建相同问题需要不同推理链的场景，评估模型如何识别未知偏好、策略性提问并调整推理过程。

Result: 评估21个前沿模型在10个任务上，发现29.0%的个性化尝试比通用回答更差，通用回答也无法有效满足个体用户需求。

Conclusion: 个性化推理需要专门开发，不会自然涌现。PREFDISCO确立了可衡量的研究前沿，揭示了当前LLMs交互能力的根本限制。
...
<details>
  <summary>Details</summary>
Motivation: 现有去偏方法评估使用不同基线和指标，导致比较不一致，且大多基于模型概率比较而非真实用户交互场景。

Method: 构建BiasFreeBench基准，比较8种主流去偏技术（4种提示方法和4种训练方法），在两种测试场景（多选QA和开放式多轮QA）下评估，引入Bias-Free Score响应级指标。

Result: 系统比较了不同去偏方法在提示vs训练范式、模型大小、以及训练策略对未见偏见的泛化能力等关键维度上的表现。

Conclusion: 建立统一测试基准，促进去偏研究的标准化评估。
...
<details>
  <summary>Details</summary>
Motivation: 现有自动化翻译评估指标缺乏可解释性和透明度，需要开发结合准确性和透明度的评估方法。

Method: 利用大型推理模型的显式推理能力，采用结构化提示模板进行系统化、逐步的翻译质量评估，评估了不同推理深度的效果。

Result: 在系统级评估中，TASER在基于参考和无参考设置下都达到了最高的软配对准确率；在段落级评估中，无参考变体在所有无参考方法中排名第一。
...
<details>
  <summary>Details</summary>
Motivation: 现有心电图-语言模型缺乏开源RAG实现和系统研究，RAG能减少幻觉并改善自然语言生成质量。

Method: 开发了首个开源ELM RAG管道，包含基线模型和消融研究，在三个公共数据集上进行实验验证。

Result: 实验表明，配备RAG的ELM在性能上持续优于非RAG基线，并揭示了关键的ELM设计考虑因素。

Conclusion: RAG管道能有效提升ELM性能，为心电图-语言模型的研究和应用提供了重要工具和基准。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM自动评分器存在局限性，因为它们基于离散偏好标签训练，无法处理主观、模糊或细微的任务。

Method: 提出两种学习方法：1) 对密集概率标签的直接监督微调；2) 对稀疏二元标签的强化学习方法。

Result: 实验结果显示，使用分布匹配目标微调的自动评分器能够产生与目标偏好分布更一致的言语化概率预测，具有更好的校准性和显著降低的位置偏差。

Conclusion: 通过校准概率自动评分器来建模完整偏好分布，可以显著提高其在主观任务上的可靠性，同时保持客观任务的性能。
...
<details>
  <summary>Details</summary>
Motivation: LLM在文本生成任务中表现出色，但在文本分类尤其是需要细微理解的修订分类任务中表现不足，且传统微调需要大量昂贵标注数据。

Method: IR-Tuning框架：基于梯度范数分布动态选择重要LLM层进行参数高效微调，冻结冗余层，实现plug-and-play的层级PEFT。

Result: 在多种文本修订任务中超越多个层级PEFT基线，实现快速收敛、低GPU内存消耗，在小规模修订语料上有效。

Conclusion: IR-Tuning为LLM在细微文本分类任务中提供了一种高效、资源友好的解决方案，特别适用于标注数据稀缺的场景。
...
<details>
  <summary>Details</summary>
Motivation: 自动生成的放射学报告在现有评估指标上得分很高，但未能获得临床医生的信任，这揭示了当前指标在评估生成报告质量方面的根本缺陷。
...
<details>
  <summary>Details</summary>
Motivation: 基于transformer的语言模型在NLP领域带来革命性进步的同时，也引入了模型透明度和可信度的重大挑战。研究人员开发了大量解释方法和评估指标，但选择最佳可解释性方法具有挑战性。

Method: 开发o-mega超参数优化工具，在社交媒体帖子与反驳声明配对的精选数据集上，通过后声明匹配管道系统性地探索不同的可解释方法及其超参数。

Result: 该工具能够自动识别最有效的可解释AI方法及其配置，在自动事实核查系统中展示了改进的透明度。
...
<details>
  <summary>Details</summary>
Motivation: SOC面临每日数万条告警的过载问题，传统检测管道脆弱且缺乏上下文，而单一LLM方法在处理嘈杂企业数据时表现不佳且透明度有限。

Method: 采用多智能体LLM架构，包括行为分析智能体检查活动序列、证据收集智能体查询外部系统、推理智能体综合发现形成可审计决策。

Result: 在多样化企业场景中，CORTEX相比最先进的单智能体LLM显著减少了误报并提高了调查质量。

Conclusion: 多智能体协作方法在SOC告警分类中比单一模型方法更有效，提供了更好的透明度和处理能力。
...
Method: 引入TokMem标记化程序记忆，将重复程序存储为紧凑可训练的嵌入。每个记忆令牌编码程序地址和控制信号，引导生成过程，具有恒定大小的开销。保持骨干模型冻结以支持持续适应。

Result: 在1000个原子回忆任务和函数调用任务上进行评估，TokMem始终优于检索增强生成，避免了重复上下文开销，且使用更少参数进行微调。
...
<details>
  <summary>Details</summary>
Motivation: 现有上下文剪枝技术如LLMLingua在通用文本上表现良好，但忽略了代码特有的结构和依赖关系，导致在编程任务中性能不佳。高API成本和生成延迟成为代码LLM处理长上下文的瓶颈。

Method: 采用双阶段压缩策略：1）粗粒度压缩，基于条件困惑度识别和排序函数级块，保留最相关函数；2）细粒度压缩，将保留函数按困惑度分段，在自适应token预算下选择最优子集以最大化相关性。

Result: 在代码补全、摘要和问答等多个任务上的评估显示，LongCodeZip始终优于基线方法，实现高达5.6倍压缩比且不降低任务性能。
...
<details>
  <summary>Details</summary>
Motivation: 个性化大语言模型输出以适应用户偏好是活跃研究领域，但之前研究主要关注分类或排序任务，未考虑需要语言和数学推理的评分预测任务，该任务具有重要工业应用价值。

Method: 通过八个模型在三个数据集上的综合实验，提供不同上下文信息，比较用户评论、一般偏好描述等对评分预测的影响，并尝试让LLM首先生成假设评论的策略。

Result: 用户评论显著提升LLM评分预测性能，效果与矩阵分解等传统方法相当；具体物品评论比一般偏好描述更有效；首先生成假设评论能进一步提升预测性能。

Conclusion: LLM在评分预测任务中表现出色，特别是结合用户评论时，为解决冷启动问题提供了有前景的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有智能体LLM方法主要依赖上下文学习，导致输入冗长且计算成本高。在专门技术微领域中，智能体微调的有效性尚不明确，因此需要探索在JP1中间件这一特定IT运维微领域中的适应性。

Method: 使用JP1特定数据集（来自领域手册）和LLM生成的推理轨迹进行微调，在推理时采用检索增强生成和上下文-答案提取器来提高信息相关性。

Result: 在JP1认证考试问题上，该方法相比基础模型实现了14%的性能提升。

Conclusion: 智能体微调在复杂微领域中具有实现领域特定推理的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态代理在图形用户界面中的自主交互能力可靠性有限，特别是在面对复杂或领域外任务时，需要探究这些代理是否在进行伪推理。

Method: 开发了Agent-ScanKit系统探测框架，包含视觉引导、文本引导和结构引导三种正交探测范式，在不需要访问模型内部的情况下量化记忆和推理的贡献。

Result: 在5个公开GUI基准测试和18个多模态代理上的结果表明，机械记忆通常超过系统推理，大多数模型主要作为训练对齐知识的检索器，表现出有限的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 传统语音对话系统使用级联流水线，会丢弃副语言线索并限制表达性。现有的端到端方法虽然减少了延迟，但仍依赖文本中介，造成根本性瓶颈。

Method: 结合模态分层架构和冻结预训练策略，在保留预训练文本LLM推理能力和知识的同时，添加原生语音能力。

Result: 在口语问答任务上达到最先进水平，在语音到语音性能上与现有文本引导系统相当，同时保持竞争力的文本性能。
...
<details>
  <summary>Details</summary>
Motivation: 随着多模态LLM驱动的智能代理在自主性和泛化能力上的进步，基于静态数据集的评估已无法充分评估它们在动态环境和多样化任务中的真实能力。现有的LLM合成数据方法主要针对LLM训练和评估设计，无法直接应用于需要工具使用和交互能力的代理任务。

Method: 使用从多源外部数据构建的知识图谱作为任务空间，通过子图采样、任务模板和元路径将语义关系转化为结构化多模态任务。采用基于节点可达性、LLM评分和相似性分析的多阶段过滤管道来保证生成任务的质量和可执行性。

Result: 实现了Graph2Eval-Bench数据集，包含1,319个涵盖文档理解和网页交互场景的任务。实验表明Graph2Eval能有效生成区分代理和模型性能的任务，揭示不同设置下推理、协作和网页交互能力的差距。

Conclusion: Graph2Eval为智能代理评估提供了新视角，能够全面评估多类型代理（单代理、多代理、网页代理）的推理、协作和交互能力。
...
<details>
  <summary>Details</summary>
Motivation: 解决RAG系统中LLM不信任提供上下文导致的幻觉问题，发现响应复制程度与上下文不忠实幻觉呈负相关

Method: 设计三种提示方法增强复制程度，通过两阶段高复制响应偏好训练获得CopyPasteLLM，使用仅365个训练样本

Result: 在FaithEval、ConFiQA和PubMedQA上表现最佳，FaithEval准确率比最佳基线提升12.2%至24.5%，仅需基线1/50的训练数据

Conclusion: CopyPasteLLM通过重新校准对内部参数知识的依赖而非外部知识来生成，有效提升上下文忠实度和幻觉控制
...
<details>
  <summary>Details</summary>
Motivation: 现有系统往往专注于孤立改进，缺乏统一的设计来确保鲁棒性和适应性，需要构建能够处理复杂现实任务的可扩展、弹性AI助手。

Method: 采用集体多智能体框架（规划、执行和批评模型投票）、分层记忆系统（工作、语义和程序层）以及精炼工具套件（搜索、代码执行和多模态解析）。

Result: 在综合基准测试中始终优于开源基线系统，性能接近专有系统水平。

Conclusion: 系统级集成对于构建可扩展、弹性且适应性强的AI助手至关重要，展示了跨领域和任务操作的能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有多语言数据集对大多数语言数据不足，导致模型在多数支持语言上表现不佳，需要为每种语言提供大量训练数据以实现高质量性能。

Method: 使用可扩展流程构建语音数据集，包括媒体检索组件和两阶段对齐算法，专门处理非逐字记录和长音频。

Result: 从22个欧洲议会提取61k+小时对齐语音，19种语言超过1k小时，22种语言超过500小时高质量数据，微调现有ASR模型后词错误率平均降低41.8%。

Conclusion: 该方法能有效构建大规模多语言语音数据集，显著提升自动语音识别性能，特别是对资源较少语言。
...
<details>
  <summary>Details</summary>
Motivation: 监督微调(SFT)的泛化能力有限，传统NLL目标在后训练场景中可能不是最优的，因为模型已经具备任务相关先验且监督信号可能冗长嘈杂。

Method: 研究概率目标家族在不同条件下的有效性，通过7个模型骨干、14个基准测试和3个领域的综合实验，发现模型能力连续体是决定目标效果的关键维度。

Result: 在模型能力较强端，偏向先验的目标（如$-p$、$-p^{10}$及其阈值变体）持续优于NLL；在模型能力较弱端，NLL占主导；中间区域没有单一目标占优。

Conclusion: 理论分析阐明了目标在连续体上的权衡关系，为根据模型能力自适应选择目标提供了原则性基础。
...
<details>
  <summary>Details</summary>
Motivation: GUI代理在处理高分辨率截图和长时程任务时面临效率挑战，现有缓存压缩方法未能充分利用GUI的空间和时间冗余特性。

Method: 结合两种新技术：空间显著性引导（用隐藏状态的L2范数增强注意力分数）和时间冗余评分（将历史帧的键投影到当前帧的键子空间来优先修剪冗余历史）。

Result: 在标准GUI代理基准测试中，GUI-KV优于竞争性KV压缩基线，在适度预算下接近全缓存准确率。在AgentNetBench基准测试中，减少38.9%的解码FLOPs同时提高4.1%的步骤准确率。
...
<details>
  <summary>Details</summary>
Motivation: 小推理模型在工具使用中经常过度思考：它们达到正确的工具参数配置后继续推理，最终用错误的调用覆盖正确结果。

Method: 引入ThinkBrake训练无关的解码启发式方法，在句子边界监控</think>与当前top token的对数概率差，当差值变小时触发终止。

Result: 在BFCL基准测试中，ThinkBrake在单轮、非实时和实时场景中保持或提高准确率，同时减少高达25%的token使用，优于各种基线方法。

Conclusion: ThinkBrake有效解决了小推理模型的过度思考问题，在保持准确率的同时显著减少计算开销。
...
<details>
  <summary>Details</summary>
Motivation: 评估大语言模型是否真正理解网络上快速传播的病毒性内容——即网络梗，特别是在中文语境下的理解能力。

Method: 构建CHIME中文网络梗数据集，包含详细标注的梗含义、起源、例句等信息；设计两个评估任务：梗解释任务（包括含义解释、起源识别、例句生成）和填空题任务（选择合适梗填入上下文）。

Result: 模型能够解释部分梗的含义，但在文化和语言细微差别较大的梗类型上表现显著下降；模型持续难以提供准确的梗起源信息；在填空题任务中模型表现低于人类水平。

Conclusion: 大语言模型对网络梗的理解能力有限，特别是在文化背景复杂的梗类型和起源追溯方面存在明显不足，CHIME数据集将为未来计算梗理解研究提供支持。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于强化学习的搜索代理方法依赖稀疏或基于规则的奖励，导致代理容易陷入次优或错误的推理路径且无法恢复。

Method: 提出ReSeek框架，包含自我修正机制（JUDGE动作）和密集的过程奖励函数（正确性奖励和效用奖励），并使用新的基准数据集FictionalHot。

Result: 实验表明，使用ReSeek训练的代理在任务成功率和路径忠实度方面显著优于现有最先进基线方法。

Conclusion: ReSeek框架通过自我修正机制和密集奖励设计，有效提升了搜索代理的性能和鲁棒性，解决了现有方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有思维链提示方法（如上下文学习和微调）成本高且效率低，需要更经济高效的推理增强方案。

Method: 引入CoT Vectors作为紧凑的推理知识表示，提出可学习的思维链向量在师生框架下优化，解决层间不稳定性问题。

Result: 在多种基准测试和模型上，CoT Vectors不仅超越现有基线，性能与参数高效微调方法相当，且需要更少的可训练参数。

Conclusion: CoT Vectors是低成本提升LLM推理的有效方法，同时作为探针揭示了LLM多步推理的功能组织机制，为理解模型推理过程提供了新视角。
...
Method: 集成基于可学习查询的多语言感知架构，结合大规模模拟代码切换数据进行预训练。

Result: 在多个语言分离基准上实现最先进性能，相对性能提升23%至52%。
...
<details>
  <summary>Details</summary>
Motivation: Tenyidie是印度那加兰邦的一种低资源藏缅语系语言，目前缺乏自然语言处理研究，特别是音节切分方面尚无相关工作报道。

Method: 创建了10,120个音节化Tenyidie词汇数据集，应用LSTM、BLSTM、BLSTM+CRF和编码器-解码器深度学习架构，数据集按80:10:10比例划分为训练集、验证集和测试集。

Result: 在测试集上，BLSTM模型取得了最高的99.21%准确率。

Conclusion: 这项工作为Tenyidie语言的形态分析、词性标注、机器翻译等NLP应用奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 当前替代文本生成面临用户标注噪声、标准不一致以及多模态大模型对上下文信息不敏感的问题，而监督微调依赖准确标注但用户生成的替代文本往往存在缺陷。

Method: 提出多维度跨模态直接偏好优化(MCM-DPO)，在单维度、配对和多偏好维度上优化偏好，涵盖文本、视觉和跨模态因素，无需精确标注即可识别更好的选择。

Result: 实验结果表明MCM-DPO方法在替代文本生成任务上持续优于DPO和SFT方法，建立了新的最先进水平。

Conclusion: MCM-DPO方法有效解决了替代文本生成中的标注质量问题，通过偏好学习提升了生成性能，同时构建了两个大规模高质量数据集支持该领域研究。
...
<details>
  <summary>Details</summary>
Motivation: 简化复杂文本对于认知障碍人群的信息公平获取至关重要，但手动创建易读文本耗时耗力，需要自动化解决方案。

Method: 采用多任务学习方法，联合训练文本摘要、文本简化和易读文本生成任务，探索了基于检索增强生成的在上下文学习和基于LoRA的参数高效微调两种策略。

Result: 在Mistral-7B和LLaMA-3-8B模型上的实验表明，多任务设置在所有配置下都优于单任务基线，RAG策略在跨域场景中表现良好，MTL-LoRA在域内配置中性能最佳。

Conclusion: 多任务学习是自动化生成易读文本的有效方法，不同策略在不同场景下各有优势，为易读文本的自动化生成提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 为认知障碍人士提供可访问性至关重要，但手动易读文本改编成本高、难以扩展。AI驱动的易读文本生成提供了可扩展解决方案，但面临数据集稀缺、领域适应和轻量级学习等挑战。

Method: 引入ETR-fr数据集，实施参数高效微调在PLMs和LLMs上建立生成基准，并开发基于自动指标和人工评估的综合评估框架，使用36个与指南对齐的问题进行评估。

Result: 总体结果显示，PLMs与LLMs性能相当，并能有效适应领域外文本。

Conclusion: 该方法为易读文本生成提供了可行的解决方案，PLMs在性能和适应性方面表现出色，为认知障碍人士的信息可访问性提供了技术支持。
...
<details>
  <summary>Details</summary>
Motivation: 现有阿拉伯语基准测试缺乏针对开放式多步推理的数据集，特别是在法律领域。

Method: 使用13K+沙特商业法庭案例，定义包括判决预测、推理链补全和相关法规识别等任务，并对阿拉伯LLMs进行基准测试和指令调优。

Result: 指令调优一个12B参数模型后，其在判决预测和阿拉伯语判决生成方面的性能显著提升，达到与GPT-4o相当的水平。

Conclusion: ALARB数据集有效提升了阿拉伯LLMs在法律推理任务中的表现，证明了其在指令调优中的实用性。
...
Method: 从英语模型开始，在相关斯堪的纳维亚语言上继续预训练（单独或合并），然后对法罗语进行微调，比较全微调和LoRA方法，构建新的最小对基准并进行人工评估。

Result: 相关语言的迁移至关重要，但最佳源语言因任务而异：冰岛语提高语言准确性，丹麦语提升理解能力；LoRA和全微调的选择也取决于任务：LoRA改善语言可接受性，全微调产生更强的理解性能。

Conclusion: 低资源语言适配需要任务特定的迁移策略，源语言选择和微调方法都应根据具体任务需求进行优化。
...
<details>
  <summary>Details</summary>
Motivation: 解决REAL-MT在真实世界嘈杂检索环境下的可靠性问题，系统评估其在噪声条件下的鲁棒性。

Method: 提出噪声合成框架和新指标，使用Qwen系列模型（包括标准LLM和大推理模型）在高中低资源语言对上进行习语翻译评估。

Result: 低资源语言对在噪声下退化更严重，LRM在纠错方面无改进且更易受噪声影响，注意力从源习语转向噪声内容，置信度增加但准确性下降。

Conclusion: 当前方法存在局限性，需要自验证集成机制，训练自由和微调策略可在清洁上下文性能为代价下提高鲁棒性，揭示了基本权衡。
...
Result: 前沿LLM在安全-实用性权衡方面表现差：许多模型为推进操作目标持续选择有害选项，而其他模型则因避免伤害而变得过度安全且无效。

Conclusion: LLM的错位并非源于无法感知伤害，而是源于优先级的错误排序。ManagerBench是评估智能体行为核心组成部分的具有挑战性的基准。
...
<details>
  <summary>Details</summary>
Motivation: 搜索增强大语言模型在复杂多跳推理中的可靠性有限，主要面临三个挑战：分解错误、检索缺失和推理错误，任一阶段的失败都会导致最终答案错误。

Method: 提出可擦除强化学习(ERL)框架，明确识别错误步骤，擦除它们并在原位置重新生成推理，防止有缺陷的逻辑在推理链中传播。

Result: 使用ERL训练的模型(ESearch)在HotpotQA、MuSiQue、2Wiki和Bamboogle上取得显著改进，3B模型比之前SOTA提升+8.48% EM和+11.56% F1，7B模型提升+5.38% EM和+7.22% F1。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在NLP任务中表现出色，但容易产生幻觉，这限制了其在现实世界应用中的可信度。需要开发专门的方法来检测和缓解检索增强生成中的幻觉问题。

Method: 结合了三个关键方法：(i)从FineWeb构建领域无关的合成数据集，通过多阶段筛选和数据重构进行精炼；(ii)生成合成的有依据和幻觉声明；(iii)使用几率比偏好优化进行偏好微调，将大模型推理能力蒸馏到小骨干网络中。

Result: 在RAGTruth子集上达到84.0%的平衡准确率，与MiniCheck(7B)和Granite Guardian 3.3(8B)等专门模型相当，但参数数量约为它们的一半。在整个基准测试中达到75.7%的平衡准确率，与GPT-4o(75.9%)等大型通用LLM相当。

Conclusion: HalluGuard证明了通过精心设计的合成数据生成和偏好优化方法，可以构建参数效率高的小型模型，在幻觉检测任务上达到与更大模型相当的性能，为可信赖的检索增强生成系统提供了可行解决方案。
...
Method: 结合分段条件风格建模与多层次对比学习来捕捉细微的人机差异，同时减轻主题依赖性；集成BIO-CRF序列标注与基于指针的边界解码和置信度校准，实现精确的跨度级检测和可靠的概率估计。

Result: 在包含10万个标注样本的新建跨学科数据集上，Sci-SpanDet实现了最先进的性能：AI-F1为80.17，AUROC为92.63，Span-F1为74.36。在对抗性重写下表现出强韧性，在IMRaD章节和不同学科间保持平衡的准确性。

Conclusion: Sci-SpanDet显著超越了现有基线方法，为解决学术文档中AI生成文本检测问题提供了有效解决方案。为确保可复现性，数据集和源代码将在发表后公开。
...
<details>
  <summary>Details</summary>
Motivation: 探索RAG在基础模型中实现专家级物理推理的潜力，特别是解决奥林匹克级物理问题的能力，受学生通过复习过往问题准备竞赛的启发。

Method: 引入PhoPile多模态数据集，包含图表、图形和方程，系统研究基于检索的推理。使用PhoPile对RAG增强的基础模型进行基准测试，涵盖大型语言模型和多模态模型，采用多种检索器。

Result: 结果显示，将检索与物理语料库集成可以提高模型性能，但同时也突显了需要进一步研究的挑战。

Conclusion: RAG在增强物理推理方面具有潜力，但仍有挑战需要解决，为检索增强物理推理的进一步研究提供了动力。
...
<details>
  <summary>Details</summary>
Motivation: 传统Best-of-N方法将LLM生成视为选择问题，只选一个最佳样本而丢弃其他多样且有价值的信息，这是零和的方法。作者希望探索协作设置，让所有候选样本都能为最终生成做出贡献。

Method: 提出Fusion-of-N方法：使用通用LLM法官来合成每个样本中最具信息量的元素，形成单一最终答案。在两种设置中应用：(i)测试时扩展：从单一模型采样并聚合；(ii)合成数据生成：融合来自多样化教师池的样本来改进学生模型。

Result: 在11种语言、3个多样化任务和不同模型规模上广泛测试，Fusion-of-N始终优于Best-of-N，在测试时扩展和合成数据生成的下游增益方面都表现出多功能性和鲁棒性。在挑战性设置下也显示出令人惊讶的优势和鲁棒性。

Conclusion: 应该改变对LLM生成的评估和利用方式，从单一质量度量转向接受其多元性质。这种转变使我们能够整合多样化优势，解锁潜在潜力，实现仅通过选择无法获得的改进。
...
<details>
  <summary>Details</summary>
Motivation: 分析大语言模型在处理不同英语方言时的性能差异，特别是标准美式英语与非标准方言变体之间的表现差距。

Method: 通过将标准美式英语问题转换为非标准方言变体，在多项选择题回答任务上测试LLM性能，并调查语法规则对性能下降的影响。

Result: 发现非标准英语问题导致准确率下降高达20%，其中三个特定语法规则（存在性"it"、零系词和"y'all"）可以解释多数方言中的性能下降。
...
Conclusion: 提出的语法引导扩散语言模型能够有效提升文本生成的质量、多样性和可控性，特别是在个性化表达方面表现出色。
...
TL;DR: 这是第一篇关于神经网络模型组件概念描述研究的综述论文，系统梳理了该新兴领域的方法、评估指标和数据集，强调了对更严谨因果评估的需求。


<details>
  <summary>Details</summary>
Motivation: 理解神经网络决策过程是机制可解释性的核心目标，特别是在大语言模型中，需要揭示底层机制并识别单个组件的作用。

Method: 通过文献综述方法，系统梳理了使用生成模型为模型组件生成自然语言概念描述的关键方法、评估指标和相关数据集。

Result: 揭示了该领域对更严谨因果评估的日益增长需求，并绘制了当前研究现状和关键挑战。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于规则的对话状态跟踪在开放域多轮聊天机器人中效率不足，无法提供复杂对话中所需的适应性和连贯性。

Method: 使用BERT进行槽填充和意图检测，XGBoost进行意图验证，GPT进行对话状态跟踪，在线代理进行实时答案生成。

Result: 在波斯语多轮对话数据集上显著提高了准确性和连贯性，优于现有方法。

Conclusion: 混合方法有效提升了对话状态跟踪能力，为更定制化、适应性和类人化的对话AI系统铺平了道路。
...
<details>
  <summary>Details</summary>
Motivation: 旨在通过整合具身智能的感知行动优势和强化学习的决策优化能力，提升文本处理的智能化水平。

Method: 提出了一种新颖的集成模型，通过理论阐述和实验探索相结合的方式进行验证。

Result: 该模型在广泛的文本处理任务中被证明非常有效，验证了其应用潜力。

Conclusion: 具身智能与强化学习的整合为文本处理领域提供了有效的解决方案，具有广阔的应用前景。
...
TL;DR: 本文对2020-2025年间非洲低资源语言自动语音识别(ASR)研究进行系统文献综述，分析了数据集、模型、评估方法等现状，发现数据稀缺、可复现性差、评估指标单一等问题，并提出未来发展方向。


<details>
  <summary>Details</summary>
Motivation: 非洲拥有2000多种语言，但低资源语言在ASR研究中严重不足，阻碍了数字包容性发展。需要系统评估现有研究状况以指导未来发展。

Method: 采用PRISMA 2020方法，检索DBLP、ACM Digital Library等数据库，筛选71篇相关研究，分析74个数据集覆盖111种语言约11,206小时语音数据。

Result: 发现只有不到15%的研究提供可复现材料，数据集许可不清晰；自监督和迁移学习有潜力但受限于预训练数据不足；主要使用WER评估，缺乏对音调和形态丰富语言的专门评估指标。

Conclusion: ASR系统发展面临数据集可用性、标注质量、许可不确定性和基准测试有限等挑战，但社区驱动倡议和方法进步提供了改进路径，未来需要利益相关者合作、伦理数据集构建、轻量模型技术和积极基准测试。
...
TL;DR: mR3是一个支持72种语言的大规模多语言奖励推理模型，在奖励建模领域实现了最广泛的语言覆盖，在多项评测中超越更大模型，同时模型尺寸小9倍。


<details>
  <summary>Details</summary>
Motivation: 现有的大型语言模型评估方法在英语中表现良好，但在非英语环境中的性能不佳，且缺乏有效的多语言训练策略研究。

Method: 提出了mR3模型，通过综合研究数据和课程选择策略，整合目标语言推理数据集，训练覆盖72种语言的奖励推理模型。

Result: 在多语言奖励模型基准测试中达到最先进性能，超越GPT-OSS-120B等更大模型，同时模型尺寸小9倍，并通过大量消融研究验证了有效性。

Conclusion: mR3模型为多语言奖励建模提供了有效的解决方案，证明了精心设计的数据和训练策略可以构建高质量的多语言奖励模型。
...
TL;DR: MASH是一个训练框架，通过强化学习惩罚外部搜索并奖励答案准确性，使LLM能够识别知识边界并选择性寻求帮助，从而产生有效的弃权行为。


<details>
  <summary>Details</summary>
Motivation: LLM无法可靠识别其参数知识边界，经常对超出边界的问题产生幻觉回答，而人类能够识别自身局限性并寻求外部帮助或弃权。

Method: 使用强化学习方法，通过按次搜索付费的奖励机制，训练LLM在需要时寻求外部帮助，不需要时直接回答。

Result: 在三个知识密集型QA数据集上，MASH显著提升了选择性寻求帮助的性能，在多跳数据集上答案准确率提高了7.6%，并能有效区分可回答/不可回答问题。
...
<details>
  <summary>Details</summary>
Motivation: 随着多模态大语言模型的流行，其组件级联架构使其继承了各组件漏洞。本研究旨在探索音频后门攻击在多模态模型中的传播机制和防御方法。

Method: 使用四个语音编码器和三个数据集，针对ASR、语音情感识别、性别和年龄预测四个任务进行后门攻击实验，并进行组件级分析识别最脆弱环节。

Result: 攻击成功率高达90.76%-99.41%，证明音频后门攻击在多模态模型中具有高度有效性。

Conclusion: 多模态模型对音频后门攻击高度脆弱，提出的微调防御方法能有效缓解预训练编码器中毒威胁。
...
TL;DR: 提出了SWF基准测试来评估LLM在社会资源分配中的表现，发现大多数模型偏向功利主义，容易受输出长度和社会影响框架的干扰，通用对话能力不能预测分配技能。
...
Method: 引入社会福祉函数基准测试，这是一个动态模拟环境，LLM作为主权分配者向异质社区分配任务，在集体效率和分配公平之间创建持久权衡。

Result: 评估了20个最先进的LLM，发现：通用对话能力不能预测分配技能；大多数LLM表现出强烈的功利主义倾向；分配策略容易受输出长度和社会影响框架的干扰。

Conclusion: 当前LLM作为社会决策者存在风险，需要专门的基准测试和针对性的AI治理对齐。
...
TL;DR: 提出GRAD方法，通过训练LLM生成特定输入的简洁演示，在预算限制下优于传统RAG方法，并在数学推理和STEM问题上表现出色。
...
Result: 在Qwen2.5-14B上，GRAD在数学推理和STEM问题上持续优于基线方法，展现出对物理、化学、计算机科学等OOD领域的强泛化能力。
...
TL;DR: 论文发现后训练对齐会导致LLM多样性减少（模式崩溃），根源是偏好数据中的典型性偏见，并提出了一种无需训练的提示策略Verbalized Sampling来规避此问题。


<details>
  <summary>Details</summary>
Motivation: 解决后训练对齐过程中LLM多样性减少（模式崩溃）的问题，发现传统方法忽略了数据层面的典型性偏见这一根本原因。

Method: 提出Verbalized Sampling策略，让模型生成一组响应及其对应概率分布，通过概率采样来增加多样性。

Result: VS在创意写作、对话模拟、开放问答等任务中显著提升多样性（创意写作中多样性提高1.6-2.1倍），且不牺牲事实准确性和安全性。

Conclusion: 提供了数据中心的模式崩溃新视角，以及实用的推理时解决方案，有助于释放预训练生成模型的多样性潜力。
...
Method: 提出SPHERE方法，使用超球面能量量化神经元均匀性，通过稀疏投影将新知识投影到与预训练权重矩阵主方向互补的空间中，减少对主方向的扰动。

Result: 在LLaMA3和Qwen2.5上的实验表明，SPHERE在编辑能力上平均优于最佳基线16.41%，同时最忠实地保持模型的一般性能。

Conclusion: SPHERE提供了一种原则性的方法来实现可靠的大规模知识编辑，通过稳定超球面能量来平衡知识保留和更新需求。
...
TL;DR: 该论文研究了自主智能体如何通过去中心化协作决策改进多智能体系统中的任务分配和协调，重点关注无人机配送和仓库自动化应用。
...
Conclusion: 自主智能体通过独立、自适应和主动行动，能够有效改善多智能体系统的任务分配和协调性能。
...
TL;DR: ToolBrain是一个轻量级框架，通过强化学习训练智能体使用工具，解决手动设计奖励、训练数据不足和多工具选择困难等问题。


<details>
  <summary>Details</summary>
Motivation: 当前训练智能体使用工具面临手动设计奖励、有限训练数据和多工具选择困难等挑战，导致适应慢、计算资源浪费和性能不佳。

Method: 采用灵活的强化学习框架，支持GRPO、DPO等RL算法和监督学习，提供自定义奖励函数和自动LLM评判系统，包含知识蒸馏、自动任务生成、工具检索等功能。

Result: 在代码智能体执行邮件搜索任务中，工具使用技能快速提升达30.0%，同时保持代码库简洁可扩展。

Conclusion: ToolBrain为研究人员和从业者提供了一个用户友好的框架，能够有效训练LLM智能体在特定领域使用工具，提升性能并节省资源。
...
TL;DR: 提出自适应推理抑制(ARS)方法，通过动态监测推理确定性来抑制冗余推理步骤，在保持精度的同时显著提升大语言模型的推理效率。
...
Method: ARS采用无需训练的方法，通过多检查点确定性估计机制和渐进抑制阈值，动态抑制冗余推理步骤。

Result: 在数学推理基准测试中，ARS实现了最高53%的token减少、46.1%的延迟降低和57.9%的能耗减少，同时保持或提高了准确率。
...
<details>
  <summary>Details</summary>
Motivation: 当前AI政策制定缺乏科学共识形成机制，而NeurIPS作为AI领域的领导者，具备推动这一进程的独特优势，能够帮助制定更高质量的AI政策。

Method: 建议NeurIPS借鉴IPCC在气候政策共识形成方面的经验，通过开展试点项目来主动催化AI政策科学共识的形成。

Result: 识别了当前AI政策共识形成机制的缺失，并论证了NeurIPS是推动这一进程的最佳选择，驳斥了AI研究者分歧过大和政策参与非NeurIPS职责的反对意见。

Conclusion: NeurIPS应该在AI政策科学共识形成方面发挥领导作用，通过借鉴IPCC的成功经验，填补当前共识形成机制的空白，从而制定更高质量的AI政策。
...
TL;DR: CERTAIN项目开发了一个综合框架，将监管合规、伦理标准和透明度整合到AI系统中，包括语义MLOps、本体驱动的数据溯源和RegOps工作流。


<details>
  <summary>Details</summary>
Motivation: AI在欧洲的快速发展带来了伦理、法律和监管方面的挑战，需要建立确保AI系统合规和透明的框架。
...
Conclusion: CERTAIN框架为AI系统提供了全面的伦理和监管透明度认证方法，有助于应对AI发展中的关键挑战。
...
TL;DR: 本文审计了视觉语言模型在保释决策预测中的表现，发现模型存在严重偏见，会错误地高置信度拒绝应获保释者。通过RAG引入法律先例和微调干预，显著提升了预测性能。


<details>
  <summary>Details</summary>
Motivation: 随着视觉语言模型的兴起，法律判决预测系统开始利用罪犯图像和文本报告，但这种方式可能带来意外后果和恶意使用。需要评估VLMs在保释预测中的表现并改进其公平性。

Method: 1. 审计VLMs在保释预测任务中的表现；2. 设计干预算法：通过RAG管道引入法律先例；3. 使用创新方案微调VLMs。

Result: 发现VLMs在多个交叉群体中表现不佳，会错误地高置信度拒绝应获保释者。通过干预措施，保释预测性能得到显著提升。

Conclusion: 本文为未来在现实世界法律判决预测中部署VLMs之前设计更智能的干预措施铺平了道路。
...
<details>
  <summary>Details</summary>
Motivation: 解决现实金融欺诈检测中证据细微且分散在多年复杂财务披露中的挑战，需要更精确的证据定位方法。

Method: 采用多智能体推理框架，整合主体级风险先验、混合检索策略和专门化智能体模块，从中国证监会执法文件和财务报告中识别和聚合跨报告证据。

Result: 在专家标注的数据集上进行的广泛实验表明，该方法在召回率和可解释性方面大幅优于通用智能体范式。
...
TL;DR: 该论文提出使用具身AI和大视觉语言模型为自主无人机提供实时情境感知和自适应决策能力，以应对突发事件的紧急降落需求。


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖安全工程师手动编写大量恢复规则，无法应对现实世界中的各种意外情况，且规则集很快变得不完整。

Method: 在Unreal Engine模拟的城市场景中，使用具身AI和大视觉语言模型让无人机动态解读周围环境并决定紧急机动策略以实现安全降落。

Result: 结果显示具身AI能够实现一类以前无法手动设计的自适应恢复和决策流程。

Conclusion: 该方法提升了自主航空系统的韧性和安全性，为无人机应急响应提供了新的技术途径。
...
TL;DR: SAA-CBR是一种新颖的神经符号图像分类方法，结合了Slot Attention的对象中心学习和基于案例推理的抽象论证。


<details>
  <summary>Details</summary>
Motivation: 整合神经网络的感知能力和符号推理的可解释性，开发一个既有效又透明的图像分类系统。

Method: 使用Slot Attention进行对象中心特征提取，然后通过AA-CBR进行符号推理，包括特征组合策略、案例库简化、基于计数的偏序等创新集成方法。

Result: 在CLEVR-Hans数据集上表现出色，与基线模型相比具有竞争力。

Conclusion: SAA-CBR是一个有效的分类器，成功融合了神经和符号方法，在保持性能的同时增强了可解释性。
...
TL;DR: Thinkquel是一个经过微调的模型，用于生成可靠、可移植且经过执行验证的数据库查询。它结合了创新的合成数据管道TS-SQL和专门设计的Token-Sequence GRPO强化学习目标，解决了自然语言到SQL转换中的模式链接和SQL方言挑战。


<details>
  <summary>Details</summary>
Motivation: 将自然语言请求转换为可靠的生产级数据转换仍然具有挑战性：正确性依赖于精确的模式链接和特定数据仓库的SQL方言，而训练期间最强的监督信号（执行成功和结果匹配）仅在序列级别提供。同时，构建大规模执行验证语料库成本高昂，且令牌级目标与这些全局信号不匹配，导致优化不稳定和可移植性有限。

Method: Thinkquel整合了创新的合成数据管道TS-SQL，利用dbt作为可移植的中间表示，以及专门设计的Token-Sequence GRPO（TS-GRPO）强化学习目标。TS-GRPO旨在在微调LLMs时弥合令牌级训练信号和序列级执行奖励之间的差距。

Result: 在500个示例的TS-SQL测试集上，Thinkquel（32B）通过两阶段SFT课程达到了93.2%的执行成功率和61.8%的精确结果匹配率，相比基础模型分别提高了67.2%（执行）和44.4%（匹配）。在Spider（14B）实验中，TS-GRPO相对于GRPO和GSPO提高了训练稳定性，并加速了执行匹配奖励的收敛。

Conclusion: Thinkquel通过其创新的合成数据管道和专门设计的强化学习目标，显著提高了自然语言到SQL转换的可靠性和可移植性，在多个基准测试中表现出色，为解决这一长期挑战提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 本地大型语言模型在工具调用场景中表现不佳，特别是在处理大规模工具集和复杂参数结构时，需要隐私保护且成本效益高的本地推理解决方案。

Method: 采用解耦微调方法，使用LoRA微调创建专用适配器，分别处理工具选择和工具特定参数生成，并通过损失掩码分离两个子任务。DualTune推理框架动态加载相应LoRA适配器进行工具调用生成。

Result: 在MCP-Bench基准测试中，使用解耦微调训练的Qwen-2.5-7B模型将基础模型的工具调用准确率提高了46%，在大多数情况下优于其他类似大小的本地推理、非推理和微调模型，甚至优于2倍大小的模型。
...
<details>
  <summary>Details</summary>
Motivation: 现有可解释性方法如StateMask存在计算成本高、探索覆盖不足、缺乏多智能体适应等问题，限制了深度强化学习在安全关键和多智能体环境中的部署。
...
Method: 使用GPT-OSS:20b模型的6个变体（1个基准模型+5种ICL配置），在840个测试中进行评估，涵盖常识问题、逻辑谜题和数学奥赛题

Result: ICL模型在常识任务上达到91%-99%准确率，但在复杂推理问题上表现下降（谜题准确率降至10-43%，基准模型为43%），数学奥赛题无显著差异
...
TL;DR: 本文研究了LLM代理在工具选择中的偏见问题，发现模型会系统性地偏好某些提供商或列表中靠前的工具，并提出了一种轻量级缓解方法。


<details>
  <summary>Details</summary>
Motivation: LLM代理依赖外部工具市场，但工具选择如果存在系统性偏见，会降低用户体验并扭曲竞争，需要评估和解决这种不公平现象。
...
TL;DR: CAIA基准测试揭示了AI在对抗性高风险环境中的关键盲点：即使是最先进的模型也无法有效应对主动欺骗，在加密市场等真实场景中表现严重不足。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试在受控环境中评估AI性能，但真实世界部署需要模型具备对抗主动欺骗的韧性。加密市场2024年因漏洞损失300亿美元，是理想的测试环境。

Method: 使用加密市场作为测试平台，评估17个模型在178个时间锚定任务上的表现，要求代理区分真相与操纵、导航碎片化信息环境，并在对抗压力下做出不可逆的财务决策。

Result: 无工具时前沿模型仅达28%准确率，工具增强后提升至67.4%，但仍低于80%的人类基准。模型系统性地优先选择不可靠的网页搜索而非权威数据，陷入SEO优化的虚假信息陷阱。

Conclusion: 当前模型尽管推理分数令人印象深刻，但在需要智力对抗主动反对的环境中仍基本不合格。对抗性鲁棒性是可信AI自主性的必要条件。
...
TL;DR: 对Transformer在潜在空间中进行递归推理的层次推理模型进行批判性回顾，提出了改进变体并在数独和迷宫任务上取得显著性能提升


<details>
  <summary>Details</summary>
Motivation: Transformer在逻辑推理方面表现不佳，可能是因为缺乏对潜在空间和递归推理等创造性应用的探索，需要深入研究层次推理模型这类新兴方法

Method: 对层次推理模型进行批判性回顾，分析关键设计选择，并提出改进变体
...
TL;DR: 提出了一个语义驱动的AI智能体通信框架，包含语义自适应传输、语义轻量化传输和语义自进化控制三大技术，以解决动态环境和有限资源下的AI智能体通信挑战。


<details>
  <summary>Details</summary>
Motivation: 随着智能服务的快速发展，通信目标从人类转向AI智能体，需要新的通信范式来实现实时感知、决策和协作。语义通信虽然提供了有前景的解决方案，但在动态环境和有限资源下的实际部署仍受限制。

Method: 1) 语义自适应传输：使用真实或生成样本进行微调，使模型适应不同环境；2) 语义轻量化传输：采用剪枝、量化和感知感知采样来降低模型复杂度和计算负担；3) 语义自进化控制：采用分布式分层决策来优化多维资源，实现动态环境中的鲁棒多智能体协作。

Result: 仿真结果表明，所提出的解决方案实现了更快的收敛速度和更强的鲁棒性，且提出的分布式分层优化方法显著优于传统决策方案。
...
TL;DR: 提出了TRACE框架，通过让智能体自由探索并将原始任务演化为更高难度的新任务，同时记录可验证的执行轨迹，解决现有智能体基准测试快速达到性能上限的问题。


<details>
  <summary>Details</summary>
Motivation: 现有智能体基准测试面临新开发的智能体快速达到性能上限的问题，难以满足评估智能体能力的需求，需要一种动态、可持续的评估系统。
...
Conclusion: 这项工作标志着从静态、人工策划的基准测试向动态、自我进化的评估系统的范式转变，为智能体开发提供了可持续且具有挑战性的发展路径。
...
TL;DR: 该论文研究了自动化评估AI系统回答患者健康问题的可行性，发现精心设计的自动化评估方法能够有效替代人工专家评审，支持AI系统的规模化比较评估。


<details>
  <summary>Details</summary>
Motivation: 当前评估AI回答患者健康问题的主要方法是人工专家评审，但这种方法劳动密集且速度慢，限制了可扩展性。自动化评估方法虽然前景广阔，但与人类判断的一致性不一且常常依赖上下文。

Method: 在100个患者案例中收集了28个AI系统的2800个回答，从三个维度进行评估：是否回答问题、是否适当使用临床笔记证据、是否使用一般医学知识。使用临床医生撰写的参考答案作为自动化评估的锚点。

Result: 自动化排名与专家评分高度匹配，表明精心设计的自动化评估方法能够有效替代人工评审。
...
TL;DR: 提出EDMS方法，通过语义丰富的状态表示和动作掩码机制，在足球等入侵性团队运动中构建可解释的玩家级智能体模型，将学习到的价值函数和策略映射到人类可理解的战术概念。


<details>
  <summary>Details</summary>
Motivation: 传统基于规则的分析方法直观但有限，现代机器学习模型虽能预测但缺乏明确的智能体表示和战术可解释性。需要构建既能从数据中学习，又具有战术可解释性和跨数据源鲁棒性的玩家级智能体模型。

Method: 提出Expandable Decision-Making States (EDMS)方法：1) 语义丰富的状态表示，在原始位置和速度基础上增加关系变量（空间得分、传球、得分等）；2) 动作掩码机制，为持球和非持球球员提供不同的决策集。

Result: EDMS与动作掩码相比基线方法，在动作预测损失和时间差分误差方面均显著降低。定性案例研究和Q值可视化显示EDMS能突出高风险高回报的战术模式（如快速反击和防守突破）。

Conclusion: EDMS方法成功构建了战术可解释的玩家级智能体模型，将学习结果映射到人类可理解的战术概念，并与多种商业和开源数据集兼容，支持跨供应商评估和可重复实验。
...
TL;DR: 该论文挑战了传统观点，发现在14个不同领域中，生成式结果奖励模型（GenORM）比过程奖励模型（PRM）更稳健，而逐步评分会因标签噪声和长推理轨迹而表现不佳。


<details>
  <summary>Details</summary>
Motivation: 评估大型语言模型在测试时扩展的可靠性通常使用外部验证器或奖励模型。传统观点认为过程奖励模型（PRM）优于结果奖励模型（ORM），但这一观点主要基于数学相关领域的证据，缺乏多领域统一评估。

Method: 在14个不同领域中统一评估了四种奖励模型变体：判别式ORM和PRM（DisORM、DisPRM）以及生成式ORM和PRM（GenORM、GenPRM）。

Result: 与传统观点相反：(i) DisORM与DisPRM表现相当；(ii) GenPRM不具竞争力；(iii) GenORM是最稳健的，在所有测试领域都获得显著且一致的提升。

Conclusion: 逐步评分会继承LLM自动标注的标签噪声，难以评估长推理轨迹（包括涉及自我纠正的推理）。理论分析显示逐步聚合会随着推理长度增长而放大错误。这些发现挑战了细粒度监督总是更好的普遍假设，支持生成式结果验证在多领域部署中的应用。
...
TL;DR: 提出VIRTUE视觉交互文本图像通用嵌入器，通过整合分割模型和视觉语言模型，使嵌入模型能够处理视觉提示（如点、边界框、掩码），实现更精确的区域级表示学习。


<details>
  <summary>Details</summary>
Motivation: 现有嵌入模型缺乏视觉交互能力，无法处理用户指定的感兴趣区域。为嵌入模型添加视觉交互不仅能解锁新的应用场景，还能学习图像中的实体级信息来补充全局表示。
...
Conclusion: VIRTUE成功将视觉交互能力引入嵌入模型，在通用和视觉交互任务上都取得了显著性能提升，为嵌入学习开辟了新方向。
...
TL;DR: 本研究开发了针对检索增强生成(RAG)系统的数据质量维度框架，通过访谈IT服务公司从业者，识别出15个数据质量维度，分布在RAG的四个处理阶段。


<details>
  <summary>Details</summary>
Motivation: 现有数据质量框架主要针对静态数据集，无法充分应对RAG系统动态、多阶段的特性，需要开发专门的数据质量维度。

Method: 对16家领先IT服务公司的从业者进行半结构化访谈，通过定性内容分析归纳推导数据质量维度。

Result: 识别出15个不同的数据质量维度，分布在RAG系统的四个处理阶段：数据提取、数据转换、提示与搜索、生成。发现新维度主要集中在早期步骤，质量问题会在管道中传播。

Conclusion: 传统数据质量框架需要扩展以覆盖RAG环境，需要采用前置质量管理和动态、步骤感知的质量管理方法。
...
<details>
  <summary>Details</summary>
Motivation: 扩散语言模型通过并行迭代去噪生成令牌，虽然降低了延迟并支持双向条件化，但其推理机制带来的安全风险尚未被充分理解，特别是针对越狱攻击的脆弱性。

Method: 研究发现如果在中间步骤出现对有害查询的肯定性令牌，后续去噪过程会被导向有害响应。基于此分析，提出了一种针对DLMs的安全对齐方法，训练模型从包含肯定性令牌的污染中间状态生成安全响应。

Result: 实验表明所提方法显著缓解了该漏洞，对任务性能影响最小，同时提高了对传统越狱攻击的鲁棒性。

Conclusion: 这项工作强调了DLM特定安全研究的必要性，扩散语言模型的安全对齐需要专门的方法来应对其独特的推理机制带来的安全挑战。
...
TL;DR: 提出了Agent Context Optimization (ACON)框架，通过优化压缩指南来压缩环境观察和交互历史，在保持任务性能的同时显著减少内存使用。


<details>
  <summary>Details</summary>
Motivation: LLM作为代理在动态环境中运行时，需要积累长历史记录，导致上下文长度增加，带来成本上升和效率下降的问题。现有上下文压缩方法主要关注单步任务或狭窄应用场景。

Method: ACON框架在自然语言空间中优化压缩指南：当完整上下文成功但压缩上下文失败时，LLM分析失败原因并相应更新压缩指南。还将优化的LLM压缩器蒸馏到更小的模型中以减少额外模块的开销。

Result: 在AppWorld、OfficeBench和Multi-objective QA上的实验显示，ACON减少了26-54%的内存使用（峰值token），同时基本保持任务性能；蒸馏到更小压缩器时保持95%以上的准确率；使更小的LM作为长视野代理性能提升高达46%。

Conclusion: ACON提供了一种有效的上下文压缩方法，能够在保持性能的同时显著减少内存使用，并通过蒸馏技术实现高效部署，提升较小语言模型在长视野任务中的表现。
...
Method: 采用受人类研究者启发的构思流程：文献挖掘识别研究趋势→探索假设设计空间→确定研究差距并证明设计选择→学习基于先前实验结果的奖励模型。

Result: HARPA生成的假设在研究提案质量上与强基线AI研究者相当，但在可行性(+0.78)和基础性(+0.85)上显著提升；在ASD代理测试中成功率更高(20 vs 11/40)；奖励模型比未训练基线提升约28%。

Conclusion: HARPA代表了AI驱动科学发现领域的重要进展，能够生成更可行、更基于文献的假设，并能从实验结果中学习改进假设质量。
...
TL;DR: 论文发现当前模型编辑方法存在根本性缺陷，其表面成功实际上基于利用隐藏捷径而非真实语义，在否定查询等简单测试中就会崩溃。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型编码了过时或不正确的知识，需要更新、删除和遗忘这些知识以解决对齐、安全等问题。模型编辑被提出作为精确编辑参数子集来更新特定事实同时保留其他知识的方法。

Method: 开发了一套新的评估方法，包括设计否定查询等负例测试，系统地检验模型编辑方法的鲁棒性。

Result: 发现最先进的模型编辑方法在简单的否定查询中就会崩溃，表明编辑很可能基于捷径而非完整语义。

Conclusion: 模型编辑当前的基础存在根本性问题，需要在进一步推进之前重新审视其可行性，因为捷径与稳健的知识整合本质上是矛盾的。
...
TL;DR: 提出CDDM方法，通过协作渐进蒸馏技术，在保持轨迹预测准确性的同时大幅减少模型参数和采样步骤，实现实时轻量化的轨迹预测。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在概率轨迹预测中表现出色，但模型规模大、采样速度慢，难以在实际自动驾驶和智能交通系统中部署。需要开发既能保持高性能又满足实时性要求的轻量化方法。

Method: 基于协作渐进蒸馏(CPD)，逐步将高容量教师扩散模型的知识转移到轻量级学生模型，同时减少采样步骤和模型规模。引入双信号正则化蒸馏损失，结合教师和真实数据的指导，防止过拟合。

Result: 在ETH-UCY行人基准和nuScenes车辆基准上达到最先进的预测精度。蒸馏后的CDDM仅需231K参数和2-4个采样步骤，实现161倍压缩和31倍加速，延迟仅9ms，同时保持96.2%的ADE和95.5%的FDE性能。

Conclusion: CDDM成功将高性能生成模型与实际部署需求相结合，为自动驾驶和智能交通系统提供了资源高效的概率预测解决方案。
...
TL;DR: 提出Expected Attention方法，通过预测未来查询对KV对的关注度来估计重要性，实现训练自由的KV缓存压缩，解决传统注意力分数方法在推理时的局限性。


<details>
  <summary>Details</summary>
Motivation: KV缓存的内存消耗是大型语言模型推理效率的主要瓶颈，传统基于注意力分数的压缩方法面临实际限制：未来token的注意力分数在压缩时不可用，且现代实现如Flash Attention不生成完整注意力矩阵。

Method: 利用LLM激活的分布特性，以闭式形式计算每个KV对的期望注意力分数，基于这些分数进行原则性排序和剪枝，最小化对残差流的影响。

Result: 该方法在预填充和解码阶段都能无缝运行，在两个场景下都持续优于最先进的基线方法。

Conclusion: 提出的Expected Attention方法有效解决了KV缓存压缩的实际挑战，同时发布了KVPress库，包含20多种技术供研究人员实现和基准测试。
...
<details>
  <summary>Details</summary>
Motivation: 在医疗等高风险领域，深度学习模型的可解释性与准确性同等重要，需要构建更透明、可解释和可信赖的AI系统。
...
TL;DR: 提出了一种迭代式相关区域缩减方法，通过重复求解同一位置并逐步限制搜索区域，引导求解器找到更小的相关区域，从而提高策略重用和剪枝效率。


<details>
  <summary>Details</summary>
Motivation: 游戏求解面临指数级增长的游戏树问题，虽然相关区域技术能减少搜索空间，但不同解会产生不同大小的相关区域。较小的相关区域更有利于重用和剪枝效率。

Method: 设计迭代相关区域缩减方法，包含三种约束生成策略，并集成相关区域模式表以充分利用过往解。

Result: 在7x7 Killall-Go实验中，平均相关区域大小减少到原始的85.95%。

Conclusion: 缩减后的相关区域可作为可重用知识永久存储，用于未来更大棋盘或不同开局时的求解任务。
...
TL;DR: 提出了自适应课程策略优化（ACPO）框架，通过动态课程和优势感知自适应裁剪机制，解决了现有策略优化算法在视觉语言模型对齐中的局限性，在多个多模态推理基准上实现了最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习算法（如PPO）在复杂推理任务中存在静态训练计划和刚性裁剪机制的问题，限制了大规模视觉语言模型的对齐效果。

Method: ACPO包含两个核心组件：1）动态课程策略，从稳定探索逐步过渡到高效利用；2）优势感知自适应裁剪（AAAC），用动态样本级边界替代固定裁剪参数。

Result: 在MathVista、LogicVista和MMMU-Pro等基准测试中，ACPO显著优于DAPO和PAPO等基线方法，实现了最先进性能、加速收敛和更好的训练稳定性。

Conclusion: ACPO通过自适应学习策略有效解决了现有策略优化算法的局限性，为复杂多模态推理任务提供了更优的模型对齐方案。
...
Method: 使用注意力机制分层编码单字和双字，通过交叉注意力融合心理健康知识图谱的领域知识，采用有序回归框架预测严重程度。

Result: 在多个数据集上，AttentionDep在分级F1分数上比现有最优方法提高超过5%，并提供可解释的预测洞察。
...
TL;DR: 提出了一种新颖的数据增强方法，通过对称性和难度两个角度增强定理证明模型的鲁棒性，并训练出在多个基准测试中达到最先进水平的7B参数非推理定理证明器EvolProver。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在形式定理证明中表现出潜力，但缺乏泛化能力且对问题表述的微小变换很脆弱，需要提升模型鲁棒性。

Method: 提出三阶段数据增强：EvolAST（基于抽象语法树的句法对称增强）、EvolDomain（跨数学领域的语义对称增强）和EvolDifficulty（难度演化增强），然后使用增强数据训练7B参数的EvolProver模型。
...
Conclusion: 提出的数据增强方法有效提升了定理证明模型的鲁棒性和性能，EvolProver在多个基准测试中超越了同类规模模型，包括推理模型。
...
TL;DR: 本文提出了DDIM反转攻击(DIA)，通过攻击DDIM轨迹路径来有效破坏恶意图像编辑，超越了之前的防御方法。


<details>
  <summary>Details</summary>
Motivation: DDIM反转技术虽然支持真实图像编辑，但也容易被恶意用户用于合成虚假内容。现有防御方法如AdvDM和Photoguard由于目标与迭代去噪轨迹不匹配，破坏效果有限。

Method: 提出DDIM反转攻击(DIA)，直接攻击集成的DDIM轨迹路径，而不是单个防御目标。

Result: DIA在多种编辑方法上都实现了有效的破坏效果，超越了之前的防御方法。
...
TL;DR: 本研究探讨了AI（特别是像ChatGPT这样的大语言模型）在教育环境中的整合，重点关注对教学的影响，通过访谈发现AI工具既能提升效率也带来学生过度依赖的挑战。


<details>
  <summary>Details</summary>
Motivation: 探索AI技术在教育领域的应用潜力及其对教学和学习过程的影响，特别是在数据科学课程中的实际效果。

Method: 通过对瓦赫宁根大学数据科学课程协调员进行访谈，收集关于AI工具使用体验的质性数据。

Result: 识别出AI工具能够简化任务和增强学习，但也发现学生过度依赖AI可能阻碍认知和问题解决能力的发展。

Conclusion: AI在教育中具有重要价值，但需要负责任地使用，调整评估方法，确保AI补充而非替代基本学习过程。
...
TL;DR: 提出了ChemX数据集，包含10个手工整理且经过领域专家验证的数据集，用于评估和改进化学信息提取方法。通过基准测试比较现有最先进的代理系统，并引入单代理方法进行文档预处理控制。


<details>
  <summary>Details</summary>
Motivation: 化学信息提取由于数据异质性而面临挑战，现有代理方法在该领域表现有限，需要专门的基准数据集来推动该领域发展。

Method: 创建ChemX数据集，进行广泛的基准测试比较ChatGPT Agent和化学专用数据提取代理，引入单代理方法控制文档预处理，评估GPT-5等现代基线模型。
...
<details>
  <summary>Details</summary>
Motivation: 研究加权知识库与c-representations这两种处理不一致知识库的形式化方法之间的语义关系，探索它们能否生成相同的解释排序。

Method: 通过语义层面的比较分析，研究加权知识库和c-representations在解释排序和蕴含关系方面的等价性。

Result: 证明在特定条件下，加权知识库和一组可废止条件可以生成相同的解释排序，两种形式化方法在语义结构上具有相对成本的等价性。

Conclusion: 两种方法在语义层面存在等价关系，这一结果对成本语义学和c-representations的进一步研究具有潜在益处。
...
TL;DR: 本文提出了一种基于逻辑一致性的无监督评估方法，通过分析分类器之间的一致与分歧来推断其性能，无需真实标签即可检测LLM评委是否违反用户设定的评分阈值。


<details>
  <summary>Details</summary>
Motivation: 当专家意见一致时无法排除任何可能的评估结果，但当意见分歧时至少有一方是错误的。这种一致与分歧的不对称性启发研究者开发无需真实标签的逻辑评估框架。

Method: 将分类器决策的一致性和分歧作为输入，构建线性规划问题，在整数空间中计算可能的正确或错误响应。使用逻辑约束（不等式）和普遍适用的线性等式（公理）来确保逻辑一致性。

Result: 开发了无需知识的警报系统，能够仅基于逻辑一致性检测LLM评委是否违反用户设定的最低评分阈值，证明了该方法在无监督评估中的实际效用。

Conclusion: 基于逻辑一致性的无监督评估方法为分类器性能评估提供了新的视角，特别是在缺乏真实标签的情况下，能够有效检测评估系统的异常行为。
...
Method: 使用电压和电流波形数据，将其分割为10-50毫秒的滑动窗口，在实时约束下评估多种经典机器学习模型。

Result: 最佳故障分类模型F1得分为0.992±0.001，最佳故障定位模型R2为0.806±0.008，平均处理时间为0.563毫秒。

Conclusion: 机器学习模型在电力系统保护中表现出色，能够满足实时处理要求，为复杂电网条件下的故障管理提供了有效解决方案。
...
TL;DR: 该研究使用SMOTE技术解决加密货币市场中Pump and Dump操纵检测的类别不平衡问题，结合集成学习模型实现了高召回率的实时检测。
...
Method: 应用SMOTE技术处理类别不平衡，并评估多种集成学习模型来区分操纵交易行为与正常市场活动。
...
TL;DR: 该论文提出了一种基于项目反应理论(IRT)的方法，通过学习LLM能力的紧凑表示来优化模型路由和性能预测。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型数量的激增，如何有效管理和利用这些庞大资源成为一个重要挑战。

Method: 采用项目反应理论框架，将模型正确回答查询的概率建模为三个因素：模型的多技能能力向量、查询的区分度向量和查询难度标量，并使用混合专家网络联合学习这些参数。

Result: 实验表明该方法在模型路由和基准准确率预测方面达到了最先进的性能，学习到的参数能够编码有意义、可解释的模型能力和查询特征信息。

Conclusion: 该方法成功地为LLM能力学习紧凑表示，为下游任务提供了有效支持，并验证了学习参数的可解释性。
...
Method: 使用蒙特卡洛树搜索(MCTS)构建AIDE框架，自动识别数据转换和模型以发现有趣的数据模式。
...
TL;DR: FusionAdapter是一种用于多模态知识图谱中少样本关系学习的模型，通过适配器模块和融合策略有效整合多模态信息，在低资源场景下优于现有方法。
...
Method: 提出FusionAdapter，包含：(1)适配器模块，使每个模态能高效适应未见过的关系；(2)融合策略，在整合多模态实体表示的同时保留模态特定特征。
...
TL;DR: 本文提出DAIL算法，通过LLM引导的进化框架自动发现数据驱动的奖励分配函数，显著提升了对抗模仿学习的稳定性和性能。


<details>
  <summary>Details</summary>
Motivation: 对抗模仿学习(AIL)方法虽然有效但被认为不稳定。现有研究主要关注密度比估计，而奖励分配函数对训练动态和最终策略性能的影响被忽视。这些函数通常基于人类设计的散度最小化目标，缺乏数据驱动的方法。

Method: 采用LLM引导的进化框架，在奖励分配函数空间中进行高效探索，自动发现基于模仿策略性能的数据驱动奖励分配函数，提出了首个元学习的AIL算法DAIL。

Result: DAIL在未见过的环境和策略优化算法上表现出良好的泛化能力，超越了当前最先进的人类设计基线方法，并实现了更稳定的训练。

Conclusion: DAIL通过数据驱动的奖励分配函数发现，显著提升了对抗模仿学习的稳定性和性能，为理解奖励分配函数在AIL稳定性中的作用提供了新见解。
...
TL;DR: 提出了一种增强训练模型推理性能的方法，通过随机性测试时搜索来改进切割平面方法中的不等式识别，特别是针对容量车辆路径问题中的圆整容量不等式和框架容量不等式。
...
Method: 引入随机边选择替代贪心方法进行图粗化，并提出GraphCHiP算法，利用粗化历史识别圆整容量不等式和框架容量不等式。

Result: 在随机生成的CVRP实例上，该方法相比现有神经分离方法能更有效地减少对偶间隙，并成功识别出有效的框架容量不等式。

Conclusion: 通过测试时搜索和随机性增强，能够显著提升训练模型在切割平面方法中的性能，特别是在识别复杂不等式方面表现出色。
...
Result: 在S&P500真实股票市场数据上测试，预测性能与ARIMA和LSTM等传统模型相当，同时提供网络内部有意义的信息流。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于大语言模型的量子电路生成方法存在两个主要挑战：(i)参数化量子门需要精确数值优化，(ii)大语言模型缺乏量子领域知识导致生成低质量电路。

Method: 提出QUASAR框架，结合工具增强的大语言模型和强化学习，设计量子电路验证方法和分层奖励机制。

Result: 在4B参数大语言模型上，QUASAR在Pass@1达到99.31%有效性，Pass@10达到100%，优于GPT-4o、GPT-5等工业级大语言模型。

Conclusion: QUASAR通过结合大语言模型和强化学习，显著提升了量子电路生成的语法和语义性能，为自动量子电路设计提供了有效解决方案。
...
TL;DR: AFFR框架整合了元学习联邦优化、能量感知客户端调度和差分隐私安全聚合，在罕见病诊断中实现了10%的准确率提升和50%以上的客户端掉线率降低。
...
Method: 采用元学习的少样本联邦优化、能量感知客户端调度策略以及带校准差分隐私的安全聚合机制。
...
TL;DR: 提出一个售后需求预测和监控的实用架构，结合统计、机器学习和深度学习模型的集成方法，包含角色驱动的分析层，用于生成评分卡和趋势诊断。


<details>
  <summary>Details</summary>
Motivation: 解决售后需求预测中的复杂挑战，包括外生信号处理、COVID-19作为特殊制度的影响，以及为不同业务角色提供可操作的见解。

Method: 使用收入感知和聚类感知的模型集成，处理外生信号（安装基数、定价、宏观指标等），采用Pareto感知分段和水平感知集成，结合LLM生成角色化叙述。

Result: 系统在90多个国家和约6000个零部件上实现了从预测到库存决策的闭环，提供校准的预测区间和业务相关的性能指标。

Conclusion: 该框架成功地将预测、监控和决策连接起来，通过可复现的工作流程和AI生成的叙述，帮助规划者从关注当前准确性转向理解准确性趋势和可操作的杠杆。
...
TL;DR: 提出SMDS方法自动发现语言模型中的特征流形，应用于时序推理案例，发现不同特征形成圆形、线性和聚类等几何结构，揭示了特征流形的功能作用。
...
Method: 引入监督多维缩放(SMDS)，一种模型无关的方法，能够自动发现特征流形。以时序推理作为案例研究应用该方法。

Result: 发现不同特征形成各种几何结构（圆形、直线、聚类）；这些结构一致反映所代表概念的属性；在不同模型家族和规模中保持稳定；主动支持模型推理；并随上下文变化动态重塑。
...
TL;DR: 该研究评估了70多个不同架构的LLM在概念表示上与人类的对齐程度，发现指令微调和注意力头维度是影响对齐的关键因素，而多模态预训练和参数大小影响有限。现有基准测试无法充分衡量人机表示对齐。


<details>
  <summary>Details</summary>
Motivation: 当前LLM的快速发展带来了多种计算要素，但尚不清楚哪些要素对构建具有人类类似表示能力的模型最为关键。现有基准测试不适合衡量人类与模型之间的表示对齐，无法可靠评估LLM在成为有用认知模型方面的进展。

Method: 使用认知科学中成熟的三元组相似性任务，评估70多个不同计算要素的模型在THINGS数据库概念上的表示，比较人类和模型的表示对齐程度。

Result: 经过指令微调的模型和具有更大注意力头维度的模型与人类表示最为对齐，而多模态预训练和参数大小对对齐影响有限。现有基准测试中MMLU比MUSR更适合捕捉表示对齐，但都无法完全解释对齐分数的方差。
...
TL;DR: 提出了一种新的前向传播范式Activation-Deactivation (AD)，通过关闭与遮挡部分对应的模型部分来消除遮挡输入特征对模型决策的影响，解决了传统黑盒可解释性方法因遮挡导致分布外图像的问题。


<details>
  <summary>Details</summary>
Motivation: 传统黑盒可解释性方法依赖遮挡输入部分产生的突变体，导致分布外图像，影响解释质量，且选择合适遮挡值需要领域知识。
...
TL;DR: 提出基于Curry-Howard对应的理论框架，将CoT推理过程映射为形式化类型证明，验证推理的忠实性。


<details>
  <summary>Details</summary>
Motivation: 解决CoT提示生成的推理过程忠实性问题，提升模型可解释性和可靠性。

Method: 利用Curry-Howard对应关系，将自然语言推理步骤转换为形式化类型证明结构。

Result: 成功将CoT推理轨迹转换为良类型证明，提供了可验证的计算忠实性证书。

Conclusion: 该框架将叙述性解释转化为可形式验证的程序，为构建更可靠AI系统提供路径。
...
TL;DR: 提出了SIRL方法，利用LLM内部的安全信念作为自生成奖励信号，无需外部验证器即可强化模型的安全拒绝行为，在多种越狱攻击下保持高防御成功率。
...
Conclusion: 有效对齐可以从内部产生，为无需大量人工监督即可扩展的自主和鲁棒AI安全机制铺平道路。
...
TL;DR: 提出一个两层的人本生产规划框架，结合约束规划（CP）和马尔可夫决策过程（MDP）来优化工业制造中的运营效率和劳动力公平性。


<details>
  <summary>Details</summary>
Motivation: 在工业制造中同时优化运营效率和劳动力公平性，将机器能力、处理时间、交货期等运营约束与工人偏好、经验、适应性和医疗限制等人因因素整合到生产规划中。

Method: 第一层使用约束规划（CP）进行订单-产线分配，生成高利用率的生产计划；第二层使用马尔可夫决策过程（MDP）进行工人-产线分配，整合人因因素。比较了贪婪分配、MCTS和RL三种解决方案策略。

Result: CP调度方法产生紧凑、可行的生产计划，延迟率低；MDP工人分配显著提高了公平性和偏好对齐度。领域专家认为两个组件都有效，并提出了改进目标函数以惩罚过度提前和改善工人分配连续性的机会。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个安全、可审计、低延迟的临床咨询系统，通过专家小组架构避免常见事件主导结果，提高急诊部门诊断的准确性和效率。

Method: 使用结构化临床事件标记化，轻量级路由器读取前几个标记并分派到专科模型（心脏血管、肺部、胃肠食管、肌肉骨骼、心理源性），每个专家继承PRISM的小型transformer骨干和标记模板。
...
TL;DR: 提出了一个基于LLM的智能体框架，用于供应链风险分析，通过将供应链网络视为知识图谱，利用网络中心性评分指导图遍历，结合上下文模板使定量数据对LLM可理解，实现实时、可解释的风险叙述生成。
...
Method: 将供应链网络建模为知识图谱，使用网络中心性评分指导图遍历器提取关键风险路径，智能体架构协调图检索、数值因子表和新闻流数据，采用上下文模板将原始数据转换为自然语言。

Result: 开发了一个轻量级方法，无需昂贵微调或专用图数据库，即可生成简洁、可解释且上下文丰富的实时风险叙述。

Conclusion: 该框架成功利用了网络与知识图谱之间的内在对偶性，为LLM驱动的金融风险分析提供了有效解决方案。
...
Method: 采用三阶段渐进训练：1)深度扩展推理能力；2)分阶段持续预训练，包括基础文本视觉理解和针对性的合成数据增强；3)高质量文本监督微调。

Result: 在Artificial Analysis Intelligence Index上获得52分，与DeepSeek-R1-0528相当；在10个图像基准测试中，性能接近Gemini-2.5-Flash和Claude Sonnet-3.7。

Conclusion: 证明通过深思熟虑的中期训练设计可以在不依赖大规模计算的情况下缩小能力差距，使前沿多模态推理对资源有限的组织变得可行。
...
<details>
  <summary>Details</summary>
Motivation: 传统并行LLM推理中，多个响应独立生成，计算资源被分割，且一个生成中的有用信息无法被其他生成利用，这与序列生成中过去计算被未来步骤使用的模式形成对比。

Method: 提出Bridge方法，将批量LLM隐藏状态视为整体张量而非独立切片，仅添加少量新参数（2.8%-5.1%），在并行生成时实现响应间的相互依赖。

Result: Bridge将基于可验证奖励的强化学习的相对平均准确率提升高达50%，并提高了正确响应的一致性。一次训练即可扩展到任何生成宽度，性能始终优于独立生成。

Conclusion: Bridge解锁了一种更通用的并行扩展模式，有效利用序列间的信息，兼容任何后生成聚合技术，为并行LLM推理提供了新的思路。
