...
TL;DR: 综述探讨了利用大语言模型（LLMs）自动化数学建模的进展，包括数据合成、模型微调、推理框架等，并分析了基准数据集的高错误率，构建了新的公平评估排行榜和在线资源门户。
...
Method: 综述了技术栈的进展，包括数据合成、模型微调、推理框架等，并清理了基准数据集，构建了新的排行榜和在线门户。

Result: 发现基准数据集错误率高，清理后构建了公平评估排行榜，并提供了在线资源门户。

Conclusion: 总结了当前方法的局限性，并指出了未来研究方向。
...
TL;DR: 亚马逊Nova AI挑战赛通过对抗性比赛推动AI安全发展，大学团队开发了先进技术，如推理对齐和多轮越狱。
...
Method: 通过对抗性比赛（红队与AI助手对话）和高质量数据支持团队开发安全技术。

Result: 团队开发了推理对齐、模型护栏等先进技术，亚马逊提供了基础设施支持。
...
Method: 通过系统回顾和比较分析，评估框架的架构、通信协议等，并识别关键问题。

Result: 建立了Agentic AI的分类体系，提出了未来研究方向。

Conclusion: 本文为下一代自主AI系统的研究与实践提供了重要参考。
...
TL;DR: 本文研究了开源深度研究代理（ODR）与闭源系统在BrowseComp-Small基准上的表现，通过改进ODR提出了ODR+模型，取得了10%的成功率。
...
Method: 使用BrowseComp-Small基准测试ODR与闭源系统，提出三项改进策略，开发ODR+模型。

Result: 原始ODR和闭源系统在测试集上准确率为0%，改进后的ODR+达到10%的成功率。

Conclusion: 开源ODR+在性能上超越了闭源系统，三项改进策略均有效。
...
TL;DR: 论文提出了一种名为LCPO的方法，通过控制输出长度来优化大型推理模型（LRMs）的效率，同时保持推理性能。
...
Method: 分析生成路径分布并通过难度估计筛选轨迹，提出基于Bradley-Terry损失框架的LCPO方法，直接平衡与NLL损失相关的隐式奖励。
...
TL;DR: KompeteAI是一个新型AutoML框架，通过动态解决方案空间探索和RAG技术提升性能，解决了传统方法的探索局限和执行瓶颈问题。
...
Method: KompeteAI引入动态解决方案空间探索，结合RAG技术从外部资源获取策略，并通过预测评分模型和加速调试方法优化执行效率。

Result: KompeteAI在MLE-Bench基准测试中平均提升3%，并加速管道评估6.9倍。

Conclusion: KompeteAI通过创新方法显著提升了AutoML的性能和效率，并在新提出的Kompete-bench中取得最佳结果。
...
<details>
  <summary>Details</summary>
Motivation: 通过引入事件熵势，量化离散事件对系统未来熵的影响，以改进AI中的不确定性建模。

Method: 将物理学中的熵势概念调整到AI领域，提出事件中心度量，并形式化定义，强调条件期望以处理反事实场景。

Result: 在策略评估、内在奖励设计、可解释AI和异常检测中展示了熵势的应用潜力。

Conclusion: 熵势框架为AI中的不确定性管理提供了理论基础，连接了热力学、信息论和机器学习。
...
TL;DR: 提出了一种基于规则的可验证逐步奖励机制（VSRM），通过奖励有效步骤和惩罚无效步骤，解决了大型推理模型（LRMs）的过度思考问题，显著提高了效率。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在复杂任务中表现出色，但存在过度思考问题，导致效率低下。现有方法需要预设预算或选择模式，缺乏灵活性和可靠性。

Method: 提出VSRM机制，基于推理轨迹中中间状态的表现分配奖励，结合PPO和Reinforce++进行实验。

Result: 在AIME24和AIME25等数学推理基准测试中，VSRM显著减少了输出长度，同时保持了推理性能。

Conclusion: VSRM有效抑制无效步骤并鼓励有效推理，从根本上缓解了过度思考问题。
...
<details>
  <summary>Details</summary>
Motivation: 解决多模态多轮问答任务中的复杂需求，包括结构化数据检索、多源信息合成和上下文理解。

Method: 1. 任务1：基于视觉大语言模型，结合GPT-4.1的知识蒸馏和课程学习策略优化强化学习。
2. 任务2和3：引入网络搜索API整合外部知识，处理复杂查询和多轮对话。

Result: 任务1以52.38%的优势排名第一，任务3排名第三，证明了方法的有效性。
...
TL;DR: HATRPO-W和HATRPO-G通过动态分配KL阈值，提升了多智能体强化学习的性能，分别实现了超过22.5%的改进。
...
Method: 提出HATRPO-W（基于KKT优化全局KL约束）和HATRPO-G（基于贪婪算法的改进-散度比分配阈值）。

Result: 实验显示两种方法显著提升HATRPO性能，收敛更快且最终奖励更高，HATRPO-W学习更稳定。

Conclusion: 动态KL阈值分配方法在异构多智能体强化学习中更灵活有效。
...
Method: 提出了TurtleSoup-Bench基准和Mosaic-Agent，并设计了多维评估协议。

Result: 实验显示LLMs在想象推理中存在明显能力限制和性能差距。

Conclusion: 研究为LLMs的想象推理提供了新见解，并为未来探索性行为研究奠定了基础。
...
TL;DR: LeanRAG提出了一种结合知识聚合和检索策略的框架，解决了现有知识图谱RAG方法中语义孤岛和检索效率低的问题。
...
Method: LeanRAG通过语义聚合算法构建实体集群和显式关系，并采用自底向上、结构引导的检索策略。
...
TL;DR: 论文提出了一种结合医学本体层次结构和电子健康记录（EHR）共现模式的框架HiRef，用于提高药物推荐的鲁棒性和泛化能力。
...
Conclusion: HiRef框架通过结合本体和共现模式，显著提升了药物推荐模型的鲁棒性和泛化能力。
...
TL;DR: MM-Food-100K是一个包含10万样本的多模态食品数据集，具有可验证来源，用于训练视觉语言模型进行营养预测。


<details>
  <summary>Details</summary>
Motivation: 提供一个公开、可追溯的高质量食品数据集，支持食品智能研究。

Method: 通过Codatta贡献模型收集数据，结合社区众包和AI辅助质量检查，并验证数据效用。
...
TL;DR: 论文提出FIRESPARQL框架，通过微调LLM、RAG和SPARQL查询修正层，解决SKG问答中LLM生成SPARQL查询的结构和语义错误。实验表明微调效果最佳。


<details>
  <summary>Details</summary>
Motivation: 解决LLM在SKG问答中生成SPARQL查询时的结构不一致和语义不准确问题。

Method: 提出FIRESPARQL框架，结合微调LLM、RAG和查询修正层，并在SciQA Benchmark上评估多种配置。

Result: 微调配置表现最佳，ROUGE-L达0.90，RelaxedEM达0.85。

Conclusion: FIRESPARQL框架有效提升SKG问答的查询和结果准确性，微调是关键。
...
Method: 引入SEQ-GPT系统，利用LLM的自然语言能力实现交互式SEQ搜索，包括动态调整搜索和用户反馈。提出了一种定制化的LLM适配流程，通过对话合成和多模型协作对齐自然语言与结构化空间数据。

Result: SEQ-GPT展示了如何通过实际数据和场景扩展空间搜索功能。

Conclusion: SEQ-GPT为空间搜索提供了更灵活和交互式的解决方案，扩展了现有服务的功能。
...
Method: 提出DxDirector-7B，一种具备深度思考能力的大语言模型，能够主导全流程诊断，并建立责任划分框架。

Result: 在罕见、复杂和真实案例中，DxDirector-7B显著优于现有医学和通用大语言模型，减少医生工作量并提高准确性。

Conclusion: DxDirector-7B标志着AI从辅助工具转变为诊断主导者，为高效、准确的临床诊断提供了新范式。
...
TL;DR: PASS是一个多模态框架，通过概率标注的决策路径和动态工具选择，解决了医疗AI中的黑盒推理、多模态整合和计算效率问题。
...
Method: PASS通过自适应采样多工具图上的代理工作流，利用任务条件分布选择工具，并设计三阶段训练优化性能与成本平衡。
...
TL;DR: 论文探讨了语言模型（LM）对齐人类偏好的方法，比较了静态数据和动态策略数据的有效性差异，并提出了一种两阶段对齐假设和边界测量算法。


<details>
  <summary>Details</summary>
Motivation: 构建可靠的AI系统需要语言模型与人类偏好对齐，但现有方法（如DPO）在动态策略数据上的有效性存在差异，需要进一步优化。

Method: 提出对齐阶段假设，将过程分为偏好注入和偏好微调两阶段，并通过理论和实验分析确定边界。

Result: 实验表明，动态策略数据在不同模型上效果差异显著（如Llama-3效果提升3倍，Zephyr降低0.4倍），验证了假设的普适性。

Conclusion: 两阶段对齐假设和边界测量算法为优化语言模型对齐提供了有效方法，适用于多种模型和方法。
...
TL;DR: 论文提出ComMCS方法，通过结合当前和后续步骤的蒙特卡洛估计器，降低估计误差，提升大语言模型在数学推理任务中的表现。
...
Result: 在MATH-500和GSM8K基准测试中，ComMCS表现优于基线方法，提升2.8和2.2分。

Conclusion: ComMCS通过降低方差有效提升推理能力，无需额外计算成本。
...
TL;DR: MSRS通过正交子空间分配和混合子空间组合策略，有效减少多属性控制中的干扰，实现精细行为调控。
...
Method: 提出MSRS框架，结合正交子空间分配、混合子空间组合和动态权重函数，实现多属性精细调控。

Result: 实验显示MSRS显著减少属性冲突，优于现有方法，并有效泛化到下游任务。
...
TL;DR: GenOM是一个基于大语言模型（LLM）的本体对齐框架，通过生成文本定义丰富本体概念的语义表示，结合嵌入模型和精确匹配工具提升对齐精度，在生物医学领域表现出色。


<details>
  <summary>Details</summary>
Motivation: 解决生物医学领域中异构知识源之间的语义互操作性和整合问题，特别是针对复杂疾病和药物相关概念。

Method: 结合LLM生成文本定义、嵌入模型检索对齐候选，以及精确匹配工具，提升本体对齐的精度。

Result: 在OAEI Bio-ML测试中表现优异，超越传统本体匹配系统和近期LLM方法。

Conclusion: GenOM通过语义增强和小样本提示，展示了其鲁棒性和适应性，为生物医学本体对齐提供了有效解决方案。
...
TL;DR: 提出了一种多智能体协作的设计评估系统（AgenticDRS），通过图匹配和提示扩展方法提升评估效果，并验证了其优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 设计评估通常需要多维度分析，但现有方法缺乏系统性协作，因此提出AgenticDRS以解决这一问题。

Method: 采用多智能体协作框架，结合图匹配的示例选择和提示扩展方法，由元智能体协调评估过程。

Result: 实验表明AgenticDRS在评估设计时优于现有基线，并能生成可操作的反馈。

Conclusion: AgenticDRS为设计评估提供了有效工具，呼吁关注这一实用但未充分探索的研究方向。
...
TL;DR: 提出了一种稀疏、目标感知的图神经网络表示方法，解决了传统密集图表示在大规模规划问题中的计算和内存问题。


<details>
  <summary>Details</summary>
Motivation: 传统方法使用全连接图表示规划状态，导致在大规模网格环境中出现组合爆炸和稀疏性问题，使得学习不可行。

Method: 采用稀疏、目标感知的图神经网络表示，选择性编码局部相关关系并显式集成目标相关的空间特征。
...
TL;DR: 论文研究了AI生成内容对人类行为和感知的影响，提出了MhAIM数据集和T-Lens系统，通过新指标和HR-MCP协议增强LLM的人类感知能力。


<details>
  <summary>Details</summary>
Motivation: 随着AI生成内容的普及，其带来的错误信息风险增加。现有研究多关注内容真实性，而较少探讨其对人类行为的影响，尤其在交易或股市等领域，预测人类反应比验证事实准确性更重要。

Method: 引入MhAIM数据集（含154,552条在线帖子，其中111,153条为AI生成），提出信任度、影响力和开放性三个新指标，并开发T-Lens系统（基于HR-MCP协议）以预测人类对多模态信息的反应。

Result: 人类研究表明，当帖子包含文本和视觉内容且两者不一致时，人们更容易识别AI生成内容。T-Lens系统通过HR-MCP协议增强了LLM的人类感知能力。
...
TL;DR: 论文通过临床试验自然语言推理基准测试，发现大语言模型在知识获取上表现优异，但在推理任务上表现不佳，揭示了其内部表征的结构性限制。
...
Method: 引入包含四种推理家族的临床试验自然语言推理基准测试，并设计GKMRV探针以区分知识获取与推理失败。评估六种当代大语言模型在直接和链式思维提示下的表现。

Result: 模型在GKMRV探针上表现优异（平均准确率0.918），但在主推理任务上表现差（平均准确率0.25），且输出推理高度一致（一致性0.87）。

Conclusion: 当前大语言模型虽具备相关知识，但缺乏结构化、可组合的内部表征，导致推理不可靠。GKMRV框架为高风险领域中的模型可靠性评估提供了有效方法。
...
<details>
  <summary>Details</summary>
Motivation: 随着AI在关键领域决策中的应用增加，其可解释性对用户理解至关重要，但视觉障碍用户的可访问性研究不足。

Method: 通过文献综述和四部分方法论（分类、角色定义、原型设计、评估）研究XAI的可访问性。

Result: 初步发现表明，简化解释比详细解释更易理解，多模态呈现有助于公平解释。
...
<details>
  <summary>Details</summary>
Motivation: 通过NLP技术提升医疗数据管理效率，但实际部署面临挑战，需分享实践经验以指导其他医疗组织。

Method: 采用迭代开发方法，结合跨学科合作，选择实用模型（包括混合方法和简单方法），并注重数据质量和错误缓解策略。

Result: 提供了可推广的实践建议，帮助医疗组织成功实施AI/NLP解决方案，改善数据管理和患者护理。

Conclusion: 通过明确业务目标、跨学科合作和实用方法，医疗组织可以更有效地部署NLP技术，提升公共健康成果。
...
TL;DR: 本文提出了一种基于区块链的透明评估协议，用于衡量开源大型语言模型（LLMs）的公平性，并在多个领域和语言中进行了验证。


<details>
  <summary>Details</summary>
Motivation: 由于LLMs在现实应用中的广泛部署，尤其是在高风险领域，公平性问题备受关注。本文旨在通过透明且可验证的方法评估LLMs的公平性。

Method: 利用智能合约在ICP区块链上执行评估，通过链上HTTP请求访问Hugging Face端点，并将数据集、提示和指标存储在链上。评估了Llama、DeepSeek和Mistral模型，使用了PISA数据集和StereoSet数据集，并扩展到多语言评估。

Result: 评估结果显示不同模型在公平性上存在差异，并揭示了跨语言的公平性差距。

Conclusion: 该方法为LLMs的公平性评估提供了透明、可验证的解决方案，并支持社区审计和长期跟踪。
...
TL;DR: 论文通过新颖的主题建模方法分析了未成年人在课堂中的匿名互动数据，提出了内容和任务的双维度分类，并展示了新应用。


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏对内容和主题的分类，且任务分类多未基于K-12真实数据，因此需要更有效的分析方法。

Method: 采用分层分类方法，结合先进的大语言模型（LLM）和预处理技术，对17,000多条消息进行分类。

Result: 发现了新的应用场景，并验证了LLM在主题建模中的优越性。

Conclusion: 研究为教育工作者和学生提供了生成式AI的使用参考，同时提出了未来研究的开放问题。
...
TL;DR: 论文介绍了INTIMA基准，用于评估语言模型的陪伴行为，发现模型普遍更倾向于强化陪伴行为，但不同模型在敏感部分表现差异显著，呼吁更一致的情感交互处理方法。
...
Conclusion: 需更一致的情感交互处理方法，以确保用户福祉。
...
TL;DR: 论文提出XFacta数据集，用于评估基于多模态大语言模型（MLLM）的虚假信息检测方法，并分析了现有方法的瓶颈，提出了半自动检测框架。


<details>
  <summary>Details</summary>
Motivation: 解决现有虚假信息检测方法在多模态数据上的瓶颈问题，以及数据集过时或人工合成的局限性。

Method: 引入XFacta数据集，系统评估多种MLLM检测策略，并提出半自动检测框架。

Result: XFacta数据集和半自动框架提升了虚假信息检测的准确性和时效性。
...
Method: 利用LLM生成合成数据，设计自动化工作流搜索能生成更有效数据的输入示例，并研究三种搜索策略。

Result: 实验表明，根据类别特性选择搜索策略的集成方法比单一策略更有效。

Conclusion: 集成方法能更高效地利用LLM生成合成数据提升分类模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有事实核查系统主要针对高资源单语环境，难以适用于印度等多语言地区的真实政治话语，因此需要开发多语言、上下文感知的工具。

Method: 引入HiFACT数据集（1500条Hinglish政治声明），并提出结合多语言上下文编码、语义对齐、证据图构建、图神经网络推理和自然语言解释生成的模型。
...
TL;DR: 研究发现，人类对词语的语义评分与大型语言模型（LLM）的嵌入矩阵中的语义关联结构相似，且均可简化为低维形式。


<details>
  <summary>Details</summary>
Motivation: 探讨人类语义评分与LLM语义嵌入结构的相似性，揭示语义特征的维度与关联。

Method: 通过对比人类评分与LLM嵌入矩阵的投影，分析语义方向（如反义词对）的相关性。

Result: LLM的语义嵌入可简化为3维子空间，与人类评分模式相似；语义特征在LLM中相互纠缠。

Conclusion: 语义信息在LLM中呈现低维结构，理解这种结构对避免语义操控的副作用至关重要。
...
TL;DR: 研究了注意力权重在生物医学文献分类中的解释性作用，发现其效果受可视化方式影响。


<details>
  <summary>Details</summary>
Motivation: 探讨注意力权重是否能为AI模型的预测提供有用的解释，并研究不同可视化方式对其效果的影响。

Method: 通过用户研究，让医学专家分类文献，评估注意力权重及其可视化方式的效果。

Result: XLNet模型分类准确，但注意力权重解释性不强，可视化方式显著影响用户感知。

Conclusion: 注意力权重的解释性效果有限，但其感知效果受可视化方式影响，用户偏好直观的展示形式。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在数学问题解答中表现出色，但在生成高质量教育问题方面仍存在挑战，EQGBench旨在推动EQG发展。

Method: 引入EQGBench基准，包含900个评估样本，覆盖数学、物理和化学，采用五维评估框架。

Result: 评估46个主流大模型，发现其在生成教育价值问题和培养学生综合能力方面仍有显著提升空间。

Conclusion: EQGBench为EQG提供了系统评估工具，揭示了未来研究方向。
...
TL;DR: 研究评估了大型语言模型在自动评分AIHQ开放式回答中的效果，结果显示模型评分与人工评分一致，且能推广到非临床数据集。


<details>
  <summary>Details</summary>
Motivation: 解决AIHQ开放式问题评分耗时的问题，探索语言模型在心理评估中的自动化潜力。

Method: 使用已有数据集，对语言模型进行微调，测试其在剩余数据上的评分一致性。

Result: 模型评分与人工评分一致，微调后表现更优，且能复现TBI与HC组的差异。
...
TL;DR: 提出了一种名为S-MoE的方法，通过任务专用令牌分配任务到独立专家网络，解决了硬参数共享中的任务干扰问题，并在语音转文本任务中验证了其有效性。
...
Method: 提出S-MoE，利用指导令牌将任务分配到独立的前馈网络，避免训练门控函数。

Result: 在语音转文本任务中，S-MoE使词错误率相对提升了6.35%。
...
TL;DR: 研究探讨了大型语言模型（LLMs）在生成和检测医疗错误信息方面的双重作用，分析了109种攻击对三种LLMs的影响，并比较了其生成的错误信息与社交媒体上的典型错误信息。


<details>
  <summary>Details</summary>
Motivation: LLMs既能生成有害的错误信息，也可能用于检测和防止其传播，研究旨在评估其在这一领域的潜力和局限性。

Method: 分析了109种针对三种LLMs的攻击，比较了攻击提示与真实健康相关查询，并评估了生成错误信息与Reddit上的错误信息的差异。

Result: LLMs能有效检测来自其他LLMs和人类的错误信息，表明其可用于构建更健康的信息生态系统。

Conclusion: 通过精心设计，LLMs可以在错误信息检测中发挥积极作用，为信息生态系统的健康做出贡献。
...
Method: 使用日本注册营养师考试题目作为提示，测试ChatGPT和三种Bing模型的准确性、一致性和响应时间，并尝试提示工程优化性能。

Result: Bing-Precise（66.2%）和Bing-Creative（61.4%）超过及格线（60%），但其他模型表现不佳；所有模型在答案一致性上存在明显不足。

Conclusion: 当前生成式AI在营养教育领域的表现尚不理想，需进一步改进以实现可靠的学习辅助工具。
...
TL;DR: 提出GG Explore框架，通过中间Guidance Graph解决LLMs在知识密集型任务中的局限性，结合结构化知识检索提升效率。


<details>
  <summary>Details</summary>
Motivation: LLMs依赖静态知识和不透明的推理过程，限制了其在知识密集型任务中的表现，而现有知识图谱方法存在粒度不匹配或上下文利用不足的问题。

Method: 引入Guidance Graph作为中间层，结合Structural Alignment和Context Aware Pruning技术，实现高效知识检索。

Result: 实验表明，该方法在复杂任务中效率更高，性能优于现有技术，且适用于较小的LLMs。

Conclusion: GG Explore通过Guidance Graph有效解决了知识检索中的粒度与上下文问题，具有实际应用价值。
...
TL;DR: Semantic Bridge是一个通用框架，用于从任意来源可控生成复杂的多跳推理问题，通过语义图编织技术显著提升LLM训练数据的质量。


<details>
  <summary>Details</summary>
Motivation: 解决高质量、推理密集型问答对稀缺的问题，特别是在稀疏领域特定来源中，现有方法无法生成可控的复杂多跳推理问题。
...
Method: 提出PersonaEval基准，使用人类撰写的对话数据，测试LLM在对话上下文中识别角色的能力，并与人类表现对比。

Result: 最佳LLM的准确率仅69%，远低于人类的90.8%，表明LLM评估器在角色扮演场景中仍不足够可靠。

Conclusion: 可靠的评估不仅需要任务特定调整，还需LLM具备类人的推理能力。PersonaEval基准已开源。
...
<details>
  <summary>Details</summary>
Motivation: 现有任务导向对话数据集多为文本或英语语音，缺乏中文语音数据及真实语音特征（如不流畅和说话人差异），限制了语音大语言模型的评估。

Method: 构建RealTalk-CN数据集（5.4k对话，60K话语，150小时），包含语音-文本双模态标注，并设计跨模态聊天任务以模拟真实交互。

Result: 实验验证了数据集的有效性，为中文语音大语言模型研究提供了坚实基础。

Conclusion: RealTalk-CN填补了中文语音任务导向对话数据集的空白，支持更全面的语音大语言模型评估与研究。
...
<details>
  <summary>Details</summary>
Motivation: 自然语言中存在许多语义相同但表达不同的句子，而现有的大语言模型（LLMs）无法为这些句子生成相同的下一词概率。本文旨在通过范畴同伦理论解决这一问题。

Method: 引入LLM Markov范畴来表示LLM生成的语言概率分布，并利用范畴同伦技术捕捉范畴中的“弱等价性”，以处理语义等价但表达不同的句子。

Result: 提出了一种新的理论框架，能够更抽象地处理LLMs中的语义等价性问题，结合了高阶代数K理论和模型范畴等理论工具。

Conclusion: 通过范畴同伦理论，本文为解决LLMs中语义等价性问题提供了新的理论视角和方法，为未来的研究奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 小型语言模型（SLMs）在复杂多变的自然语言中难以提取核心问题并进行有效推理，限制了其性能。
...
TL;DR: FedCoT框架通过轻量级思维链增强机制和客户端分类器感知的聚合方法，在联邦学习中提升LLM的推理能力，同时确保隐私和资源效率。


<details>
  <summary>Details</summary>
Motivation: 在医疗等敏感领域，现有联邦学习方法无法同时满足推理准确性、可解释性和隐私保护需求，亟需一种新方法。

Method: 提出FedCoT框架，结合本地模型生成多推理路径和动态选择最优路径的判别器，采用改进的LoRA模块聚合方法。

Result: 实验表明，FedCoT在医疗推理任务中显著提升性能，同时保持隐私和资源效率。

Conclusion: FedCoT为联邦学习中的LLM推理提供了一种高效、隐私安全的解决方案，特别适用于医疗领域。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在学科问答中存在幻觉和非事实生成问题，影响回答的可靠性。共形预测和显著性测试的协同整合尚未被探索。
...
<details>
  <summary>Details</summary>
Motivation: 现有TTC策略（如TTT和RAG）对不同查询效果不一，盲目应用会导致高计算成本，需自适应选择最优策略。

Method: 提出RTTC框架，利用预训练奖励模型动态选择TTC策略，采用分布式架构和查询状态缓存优化计算效率。
...
TL;DR: 本文提出了一种结合自然语言处理、机器学习和大型语言模型的智能产后抑郁症（PPD）筛查系统，旨在实现实时、低成本、非侵入性的语音分析，并解决模型可解释性问题。


<details>
  <summary>Details</summary>
Motivation: 产后抑郁症（PPD）严重影响母亲的身心健康，快速检测PPD及其风险因素对及时干预至关重要。本文旨在利用最新技术帮助从业者进行实时筛查和治疗推荐。
...
Result: 在多个基准测试中表现优异，如FastThink模式在MATH上减少65.4%推理长度并提升3.6%准确率。

Conclusion: SABER在预算限制下实现高效推理，并具备跨领域泛化能力。
...
TL;DR: 研究开发了一种结合Transformer嵌入和手工特征的方法，用于通过语音检测阿尔茨海默病，并测试了数据增强和多模态模型的性能。
...
Method: 结合Transformer嵌入与手工特征，测试LLM生成合成语音的数据增强，评估单模态和多模态LLM分类器。

Result: 融合模型F1=83.3（AUC=89.5），数据增强后F1提升至85.7。多模态模型性能较低（GPT-4o F1=70.2）。

Conclusion: 结合Transformer和手工特征可提升ADRD检测，LLM在分类和数据增强中有效，但多模态模型需进一步改进。
...
<details>
  <summary>Details</summary>
Motivation: 个性化文本生成需要更关注用户个体差异，但现有评估方法常忽略这一点。
...
TL;DR: LFJ是一种基于隐藏状态插值的攻击方法，通过优化查询对和梯度引导插值，成功绕过LLMs的安全对齐，攻击成功率高达94.01%。同时提出对抗训练防御方法，显著降低攻击成功率。
...
Method: LFJ通过选择高相似性的查询对，在关键层和令牌上进行梯度引导插值，并优化攻击成功、输出流畅性和计算效率。

Result: 在Vicuna和LLaMA-2等模型上，LFJ的平均攻击成功率为94.01%，优于现有方法。对抗训练防御可将攻击成功率降低80%以上。

Conclusion: LFJ是一种高效的越狱攻击方法，同时提出的防御策略能有效缓解其威胁，为LLMs的安全性提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 现有提示优化方法未考虑推理策略，而推理策略与提示优化之间存在强依赖关系，且用户偏好对提示和推理配置的选择有重要影响。

Method: 提出IAPO框架，联合优化提示和推理规模，开发PSST算法进行固定预算训练，并分析有限预算下的错误概率保证。

Result: 在六项任务（包括多目标文本生成和推理）上验证了PSST的有效性，证明了推理感知在提示优化中的关键作用。

Conclusion: IAPO框架通过联合优化提示和推理规模，显著提升了黑盒LLM的对齐效果，强调了推理感知的重要性。
...
TL;DR: 研究发现LLMs的思考模式更容易被Jailbreak攻击攻破，攻击成功率高于非思考模式。通过分析样本，发现教育目的和过长的思考长度是攻击成功的特征。本文提出了一种安全思考干预方法，通过添加特定思考标记来显式指导LLMs的内部思考过程，显著降低了攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 思考模式被认为是LLMs中最有价值的模式之一，但研究发现其容易被攻击，导致LLMs给出有害答案。为了解决这一问题，提出了安全思考干预方法。

Method: 在提示中添加特定思考标记，显式指导LLMs的内部思考过程，以减少攻击成功率。

Result: 安全思考干预方法显著降低了LLMs在思考模式下的攻击成功率。

Conclusion: 通过安全思考干预方法，可以有效减少LLMs在思考模式下被攻击的风险，提升其安全性。
...
TL;DR: APIE框架通过双重不确定性指标（格式不确定性和内容不确定性）主动选择最具挑战性的样本作为few-shot示例，显著提高了信息抽取的准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统选择策略忽略了模型在生成结构化格式时的困惑，导致信息抽取性能不佳。

Method: 提出APIE框架，通过双重不确定性指标（格式和内容）评估模型困惑，并主动选择最具挑战性的样本作为示例。

Result: 在四个基准测试中，APIE显著优于基线方法，提高了抽取准确性和鲁棒性。

Conclusion: APIE强调了双重不确定性指标在构建可靠结构化生成系统中的重要性。
...
Method: 设计了mSCoRe基准，包含推理技能分类、数据合成管道和复杂度扩展框架，评估了八种先进大语言模型。

Result: mSCoRe对当前模型具有显著挑战性，尤其在更高复杂度下，模型在多语言和文化常识推理中存在局限。

Conclusion: 研究为改进多语言常识推理能力提供了方向，强调了未来模型需进一步优化的领域。
...
TL;DR: 论文提出了一种新的多轮对话基准测试，用于评估大型语言模型在复杂交互任务中的表现，揭示了当前模型的不足。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在清晰完整的问题中表现优异，但在复杂交互任务中表现不佳，因此需要开发能够处理多轮对话和信息搜索的模型。

Method: 引入了一套多轮任务基准测试，每个任务设计用于测试特定能力（如推理、对话和信息搜索），并采用确定性评分机制。

Result: 评估显示当前模型在指令遵循、推理和规划方面存在显著不足。

Conclusion: 该基准测试为未来研究提供了改进模型在复杂交互任务中能力的平台。
...
TL;DR: LaaJMeter是一个基于模拟的框架，用于在特定领域中对LLM-as-a-Judge（LaaJ）进行受控元评估，解决领域特定环境下评估指标未经验证的问题。


<details>
  <summary>Details</summary>
Motivation: 在领域特定环境中，LLM作为评估者（LaaJ）面临标注数据稀缺和专家评估成本高的问题，现有评估指标未经验证，难以确定其有效性和阈值。
...
TL;DR: 论文提出了一种翻译难度估计任务，并开发了新模型Sentinel-src，用于识别机器翻译系统可能表现不佳的文本，从而改进评估和未来研究。


<details>
  <summary>Details</summary>
Motivation: 当前机器翻译质量已接近完美，难以区分模型优劣或识别改进方向，因此需要自动识别翻译困难的文本。

Method: 定义了翻译难度任务，提出新评估指标，并开发了Sentinel-src模型，对比了启发式方法和LLM评估方法。

Result: Sentinel-src模型在翻译难度估计上优于启发式方法和LLM评估，并发布了两个改进模型Sentinel-src-24和Sentinel-src-25。
...
<details>
  <summary>Details</summary>
Motivation: 提高大型语言模型和视觉语言模型的透明度和可解释性，避免传统方法的高计算成本。
...
TL;DR: PakBBQ是一个针对巴基斯坦文化和区域背景的偏见基准数据集，用于评估多语言LLMs的公平性，结果显示语境明确化和负面提问能显著减少偏见。
...
Result: 实验显示明确语境提升12%准确率，乌尔都语表现优于英语，负面提问减少刻板回答。

Conclusion: 强调语境化基准和简单提示工程在低资源环境中减少偏见的重要性。
...
TL;DR: 研究比较了四种深度学习架构（前馈网络、CNN、Transformer、BERT）在短文本序列中预测表情符号的效果，发现BERT表现最佳，CNN在罕见表情符号类别上更有效。
...
TL;DR: 利用大型语言模型（LLM）从临床访谈转录本中预测精神分裂症高危患者的BPRS评分，结果接近人类评估的可靠性。
...
Method: 使用LLM从409名高危患者的访谈转录本中预测BPRS评分，评估其零样本性能和在多语言、纵向信息中的表现。

Result: LLM预测的BPRS评分与真实评估高度一致（中位数一致性：0.84，ICC：0.73），在多语言中表现更优（一致性：0.88，ICC：0.70）。

Conclusion: LLM在标准化和改善高危患者症状评估方面具有潜力，尤其是在多语言和纵向数据分析中。
...
TL;DR: 该研究通过计算和语料库方法，探讨了Toki Pona（一种约120个核心词的构造语言）的语言变化和变异，发现社会语言学因素对其影响与自然语言相似。
...
Method: 采用计算和语料库方法，分析词类流动性和及物性等特征，考察不同句法位置的内容词偏好变化及不同语料库的用法变异。

Result: 结果表明，Toki Pona受社会语言学因素影响，且构造语言系统会因社区使用而自然演化。
...
<details>
  <summary>Details</summary>
Motivation: 研究提示工程时发现LLM对提示词微小变化敏感，部分源于其归纳偏置。利用LLM输出作为提示词的一部分，可更高效设计匹配其偏置的提示词。

Method: 提出归纳偏置提取与匹配策略（Inductive Bias Extraction and Matching），利用LLM自身输出优化提示词设计。

Result: 实验显示，该策略使分类任务的Likert评分提升19%，排序任务提升27%。

Conclusion: 通过匹配LLM的归纳偏置，可显著提升提示词效果，为提示工程提供新思路。
...
<details>
  <summary>Details</summary>
Motivation: 当前偏见检测方法多为定量且自动化，忽略了语言中偏见的微妙表现，因此需要定性方法补充。

Method: 通过手动分析LLM生成的关于黑人和白人女性的短故事，研究性别和种族偏见。

Result: 黑人女性被描绘为与祖先和抵抗相关，白人女性则表现为自我发现过程；模型修正偏见时仅提供表面修改。
...
Method: 结合ArXiv-MCP检索增强的上下文生成管道、监督微调以及强化学习复合奖励函数。
...
Result: 在真实PDF文档的评估中，DOTABLER的Precision和F1分数超过90%，性能优于GPT-4o等先进模型。
...
<details>
  <summary>Details</summary>
Motivation: LLM推荐系统过度强调语义关联，导致协同信号在传播过程中逐渐减弱，影响性能。

Method: 提出FreLLM4Rec，结合全局图低通滤波器（G-LPF）去除高频噪声，并通过时频调制（TFM）逐层保留协同信号。

Result: 在四个基准数据集上，FreLLM4Rec性能提升高达8.00%（NDCG@10）。

Conclusion: FreLLM4Rec有效缓解协同信号衰减，为改进LLM推荐系统提供了理论支持。
...
Result: 在SIB-200基准测试中，XPE对低性能语言效果显著，而混合机制在多语言场景中更具适应性。
...
TL;DR: 两阶段微调方法使Qwen3 14B大模型在韩语中实现原生思维，结合监督微调和强化学习，显著提升韩语逻辑推理和问题解决能力。


<details>
  <summary>Details</summary>
Motivation: 提升大语言模型在韩语中的逻辑推理能力，使其能够完全以韩语进行内部思维链。

Method: 第一阶段使用监督微调（SFT）在高质量韩语推理数据集上建立基础；第二阶段采用定制化的GRPO算法进行强化学习，并通过引入裁判模型解决稳定性问题。

Result: 模型在韩语任务和通用推理能力上均有显著提升，尤其在数学和编程任务中表现突出。

Conclusion: 该方法成功实现了模型在韩语中的原生思维，同时保持了知识和语言能力。
...
TL;DR: 提出了一种新的序列到序列方法，用于跨语言ABSA任务，无需依赖翻译工具，性能提升达10%。
...
Method: 使用约束解码的序列到序列方法，避免外部翻译工具。
...
TL;DR: 该论文探讨了捷克语文本摘要任务，尤其是历史文档摘要，通过使用Mistral和mT5等大型语言模型，在现代和历史捷克语数据集上取得了突破性成果。
...
Result: 在现代捷克语摘要数据集SumeCzech上取得最新最优结果，并引入历史文档数据集Posel od Čerchova。
...
TL;DR: 本文提出了一种基于序列到序列模型的约束解码方法，用于跨语言方面情感分析（ABSA），无需依赖翻译工具，性能提升5%。同时支持多任务处理，效果提升超10%。
...
<details>
  <summary>Details</summary>
Motivation: 现有恶意内容检测方法依赖人工标注或LLM，效率或准确性不足，需平衡两者。
...
Method: 提出SFPF框架，利用稀疏自编码器重构隐藏层表示，对高激活特征进行扰动生成对抗性文本，同时保留恶意意图并放大安全信号。

Result: 实验表明，SFPF生成的对抗性文本能够绕过现有防御机制，揭示了当前NLP系统的持续漏洞。

Conclusion: 该方法在对抗有效性和安全性之间取得了平衡，但其效果因提示和层而异，且对其他架构和更大模型的通用性尚待验证。
...
TL;DR: ComoRAG是一种动态、迭代的检索增强生成方法，针对长篇叙事理解中的复杂推理问题，通过多轮检索和记忆整合提升性能。


<details>
  <summary>Details</summary>
Motivation: 解决传统RAG方法在长篇叙事理解中因单步检索和静态记忆导致的动态关系捕捉不足问题。

Method: 提出ComoRAG，通过动态记忆工作空间和多轮推理循环，生成探测查询并整合新证据到全局记忆池。

Result: 在四个长上下文叙事基准测试中，ComoRAG比最强基线性能提升11%。

Conclusion: ComoRAG为基于检索的长上下文理解提供了一种认知启发的状态推理范式，特别适用于需要全局理解的复杂查询。
...
TL;DR: 论文提出了IdiomEval框架，用于评估中文成语翻译质量，发现现有系统（如GPT-4和Google翻译）在成语翻译上表现不佳，错误率高达28%。同时，现有评估指标与人工评分的相关性较低（Pearson<0.48）。作者改进了模型，错误检测F1分数达到0.68。


<details>
  <summary>Details</summary>
Motivation: 中文成语的翻译因其独特的结构和历史背景而具有挑战性，但现有机器翻译系统对其表现不佳，且缺乏有效的评估方法。

Method: 提出IdiomEval框架，构建包含900个翻译对的标注数据集，涵盖多个领域和系统（如GPT-4和Google翻译），并开发改进模型。

Result: 现有系统在成语翻译上错误率高（如GPT-4错误率28%），现有评估指标与人工评分的相关性低（Pearson<0.48）。改进模型的错误检测F1分数为0.68。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）的计算成本高昂，需要一种方法在资源受限时优化计算分配。

Method: 将LLM视为资源受限的代理（注意力头和神经元块）的经济体，通过激励驱动训练引入可微计算成本项，促进稀疏和高效激活。

Result: 在GLUE和WikiText-103任务上，该方法显著减少FLOPS和延迟，同时保持准确性，且注意力模式更可解释。
...
TL;DR: 论文研究了多模态大语言模型（MLLMs）中的文本主导问题，提出了两种评估指标（MDI和AEI），并分析了其三大成因，最后提出了一种有效的标记压缩方法。
...
Method: 通过提出MDI和AEI两种指标量化文本主导程度，并分析其成因（标记冗余、融合架构设计和任务形式）。提出了一种标记压缩方法以平衡模型注意力。

Result: 研究发现文本主导问题在所有测试模态中普遍存在。提出的标记压缩方法显著降低了LLaVA-7B模型的MDI值（从10.23降至0.86）。

Conclusion: 本文为开发更均衡的多模态语言模型提供了分析框架和方法，解决了文本主导问题。
...
Method: DiFaR使用五种思维链提示从大型视觉语言模型中生成多样解释，并通过轻量级后过滤模块选择事实性和相关性高的句子。
...
TL;DR: 论文提出了一种结合图基方法和Transformer架构的连续孟加拉手语翻译方法，显著提升了翻译性能，并在多个数据集上实现了新的最优结果。
...
TL;DR: 本文研究了NLP中隐私与可解释性之间的权衡关系，发现两者可以共存，并提出了未来研究的实用建议。


<details>
  <summary>Details</summary>
Motivation: 尽管可解释性和隐私保护的NLP研究日益增多，但两者交叉领域的研究仍不足，本文旨在填补这一空白。

Method: 采用差分隐私（DP）和后验可解释性方法，实证研究了隐私与可解释性的权衡关系。

Result: 发现隐私与可解释性的关系复杂，受下游任务性质、文本私有化及可解释性方法选择等因素影响。
...
TL;DR: 论文提出了一种名为VAC的新框架，通过自然语言反馈（NLF）替代传统的标量奖励，以提升个性化问答模型的性能。
...
Method: VAC框架使用自然语言反馈（NLF）作为监督信号，交替优化反馈模型和策略模型。

Result: 在LaMP-QA基准测试中表现优于现有技术，人类评估也证实了生成回答的优越性。
...
TL;DR: MS-GRPO算法通过多步信用分配和优势加权采样策略，提升了小型LLM在序列决策任务中的性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）在序列决策任务中潜力巨大，但依赖大模型导致计算成本高，需要改进小型模型。现有方法无法处理多步任务中的信用分配问题。

Method: 提出MS-GRPO算法，基于TSMG和LAP框架，通过累积奖励分配和绝对优势加权采样策略优化小型LLM。

Result: 在Snake和Frozen Lake任务中，3B参数模型表现优于72B基线模型50%。
...
TL;DR: 该论文探讨了欧洲深度推理框架（eDIF）的可行性，旨在支持大型语言模型的机制可解释性研究，并通过试点研究评估其性能和实用性。
...
Result: 用户参与度逐步提升，平台性能稳定，远程实验功能受到好评，但也存在数据下载时间长和执行中断的问题。

Conclusion: 该项目为欧洲LLM可解释性基础设施的普及奠定了基础，并为未来扩展和社区合作提供了方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究在语言使用质量、数据稀缺和模型解释方面存在不足，需要更透明和可解释的评估方法。

Method: 采用特征工程、数据增强和SHAP分析，优先使用透明特征，避免“黑箱”预测。

Result: 在新英汉连续口译数据集上表现优异，识别出BLEURT、CometKiwi分数对忠实度、停顿相关特征对流畅度、中文特定短语多样性对语言使用的预测作用。
...
TL;DR: 首次系统研究将科普特语翻译为法语的策略，评估了多种方法，并证明多样化和噪声感知的训练语料能显著提升翻译质量。
...
Method: 系统评估了直接翻译与枢轴翻译、预训练的影响、多版本微调的益处以及模型对噪声的鲁棒性，使用对齐的圣经语料库。

Result: 多样化和噪声感知的训练语料显著提升了翻译质量。
...
TL;DR: 扩散语言模型（DLMs）作为自回归模型的替代方案，通过并行生成和迭代去噪过程，降低了推理延迟并捕获双向上下文，性能接近自回归模型。本文综述了DLMs的发展、技术、优化及应用，并讨论了其局限性和未来方向。


<details>
  <summary>Details</summary>
Motivation: 探索扩散语言模型（DLMs）作为自回归模型的替代方案，分析其优势（如并行生成、双向上下文捕获）及性能表现，为研究者和实践者提供全面参考。

Method: 综述DLMs的发展历程、技术原理、优化策略（如解码并行化、缓存机制）及多模态扩展，涵盖预训练和训练后方法。

Result: DLMs在推理速度和生成质量上接近自回归模型，适用于多种自然语言处理任务，并展示了多模态应用的潜力。

Conclusion: DLMs是一种有前景的替代方案，但仍面临效率、长序列处理等挑战，未来研究需进一步优化和扩展其能力。
...
TL;DR: ICE框架通过双向注意力机制和迭代优化，将前缀提示转为原位提示，显著提升模型性能并减少计算开销。


<details>
  <summary>Details</summary>
Motivation: 传统大语言模型的前缀提示和顺序生成限制了双向信息的灵活性，需要更灵活的提示策略。

Method: 提出ICE框架，结合原位提示和置信度感知的提前退出机制，优化迭代过程。

Result: 在GSM8K上准确率提升17.29%，速度提升4.12倍；MMLU上加速276.67倍，性能保持竞争力。

Conclusion: ICE为扩散大语言模型提供了高效灵活的原位提示策略，显著提升性能与效率。
...
<details>
  <summary>Details</summary>
Motivation: 在高负荷领域（如NLP）中，评审能力有限，新颖性评估是同行评审中未被充分研究的关键环节。

Method: 采用三阶段方法：内容提取、相关文献检索与综合、结构化比较评估，基于大规模人类评审分析建模。
...
Result: 7B Psyche-R1在多项心理基准测试中表现优异，接近671B DeepSeek-R1。

Conclusion: Psyche-R1为心理领域提供了可靠且专业的解决方案，展示了结合推理与共情的潜力。
...
TL;DR: 论文探讨了利用大型语言模型（LLMs）作为强化学习（RL）中代理搜索任务的高效模拟器，以减少对外部搜索引擎的依赖。通过Self-Search和SSRL方法，LLMs展示了强大的内部知识利用能力。
...
Method: 提出Self-Search量化LLMs的搜索能力，并通过SSRL结合格式和规则奖励优化其内部知识利用。
