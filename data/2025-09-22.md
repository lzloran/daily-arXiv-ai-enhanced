...
Motivation: 标准预训练只学习单文档内的token因果相关性，无法有效建模丰富的文档间相关性，而这些相关性可能带来更好的性能。

Method: SBP首先从预训练数据集中学习文档间关系模型，然后利用该模型合成大量新语料进行联合训练。在计算匹配的预训练设置下，从头预训练了一个30亿参数的模型，使用了高达1万亿个token。

Result: SBP持续优于强重复基线，实现了通过访问20倍独特数据可达到的性能改进的显著部分。合成文档超越了简单的改写，能够从种子材料中抽象核心概念并在此基础上构建新叙述。

Conclusion: SBP不仅具有强大的实证性能，还自然地符合贝叶斯解释：合成器隐式地学习了相关文档间共享的潜在概念抽象。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在英语等高资源语言中表现良好，但对宗喀语等低资源语言的分词效果不佳。宗喀语作为不丹的国语，仅有约70万使用者，其语言复杂性给NLP带来独特挑战，需要专门的分词方法研究。

Method: 研究比较了三种主流分词算法：字节对编码（BPE）、WordPiece和SentencePiece（Unigram），使用子词生育率、连续词比例、归一化序列长度和执行时间等指标评估它们在宗喀语上的性能。

Result: 实验结果显示，三种算法都显示出潜力，但SentencePiece在宗喀语分词方面表现最为有效，为构建宗喀语大型语言模型奠定了基础。

Conclusion: 研究强调了为低资源语言定制分词方法的必要性，SentencePiece算法为宗喀语NLP的进一步发展提供了有效解决方案，需要持续的研究投入。
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型的安全性研究在低资源多语言环境中探索不足，特别是在新加坡这样的多元语言文化背景下，需要系统评估LLM的安全防护能力。

Method: 采用红队测试方法，在对话、问答和内容创作三个真实场景下，使用包含新加坡英语、中文、马来语和泰米尔语的SGToxicGuard数据集对多语言LLM进行系统性漏洞探测。

Result: 实验结果显示，当前最先进的多语言LLM在新加坡多语言环境中的安全防护存在严重漏洞，无法有效应对文化敏感性和毒性内容。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型越来越多地用于需要客观评估的应用中，但政治偏见可能影响其客观性。虽然许多研究发现LLMs偏好左倾立场，但政治偏见对事实核查等下游任务的影响尚未充分探索。

Method: 通过将德语声明中的词汇替换为委婉语或贬义词，构建事实等效但政治内涵不同的最小对，评估六个LLMs在分类这些声明为真或假时的一致性。

Result: 研究发现，相比政治倾向，判断性词汇的存在显著影响真实性评估。虽然少数模型显示出政治偏见倾向，但通过在提示中明确要求客观主义并不能缓解这种偏见。

Conclusion: 政治偏见在LLMs的事实核查任务中影响有限，判断性词汇的存在比政治倾向本身对模型评估的影响更大，且现有的缓解方法效果不佳。
...
Method: 提出近似问题侧效应(AQE)量化问题意识贡献，并开发SCAO(单词回答语义压缩)方法增强模型侧信号利用。

Result: 分析显示现有成功多来自问题侧模式利用，SCAO在减少问题侧线索时表现更稳定一致。

Conclusion: SCAO方法能有效促进LLM真正的自意识发展，减少对问题侧捷径的依赖。
...
<details>
  <summary>Details</summary>
Motivation: 现有机器生成文本检测方法主要关注区分人类或机器撰写，忽略了机器参与的细粒度用途（如润色、翻译），而这些不同用途可能具有不同的意图和风险。

Method: HERO采用分层长度鲁棒检测框架，结合长度专业化模型的预测结果，并使用子类别指导模块来分离易混淆的细粒度类别（如不同源语言）。

Result: 在5个LLM和6个领域上的实验表明，HERO比现有最先进方法平均提升2.5-3 mAP。

Conclusion: HERO能够有效检测机器参与文本的细粒度类型，为理解LLM使用意图提供了更精确的工具。
...
Method: 通过反事实示例区分核心语义与虚假上下文进行训练阶段去偏，并采用混合专家架构进行动态路由选择模态特定去偏专家。
...
<details>
  <summary>Details</summary>
Motivation: 为西非代表性不足的沃洛夫语开发首个语音语言模型，扩展其语音识别和翻译能力。

Method: 收集大规模自发高质量语音数据，在HuBERT上持续预训练，将语音编码器集成到沃洛夫语LLM中，并探索多步思维链推理。

Result: 模型在语音识别上优于基础模型和非洲中心模型，在语音翻译方面表现良好。

Conclusion: 成功开发了首个沃洛夫语语音LLM，模型和代码将公开分享。
...
<details>
  <summary>Details</summary>
Motivation: 解决低资源语言自动语音识别数据稀缺的问题，利用现有标注数据生成更多训练样本。

Method: 使用三种文本生成方法（基于词汇替换、随机替换、LLM生成）生成新文本，然后通过TTS合成音频，将合成数据与原始数据结合用于模型微调。

Result: 在四种低资源语言上显著提升性能，其中Nashta语言WER绝对降低14.3%，方法在高资源语言如英语上也有效。

Conclusion: 提出的数据增强方法具有广泛适用性，能够有效解决低资源ASR的数据稀缺问题。
...
<details>
  <summary>Details</summary>
Motivation: 尽管自然语言解释方法能够以自解释的方式解释大语言模型的行为，但目前缺乏对这些生成解释的有效不确定性量化方法，这对于理解解释背后的置信度至关重要。

Method: 提出了一种新颖的不确定性估计框架，采用事后处理和模型无关的方式；同时设计了一种鲁棒的不确定性估计方法，能够在噪声环境下保持有效的不确定性保证。

Result: 在问答任务上的大量实验证明了所提出方法的理想性能。

Conclusion: 该工作填补了自然语言解释不确定性量化的研究空白，为理解大语言模型解释的置信度提供了有效工具。
...
<details>
  <summary>Details</summary>
Motivation: 随着医学影像数量的增加，复杂医学文本的自动摘要工具需求日益增长，但在敏感且数据受限的医学领域，抽象摘要仍然具有挑战性。

Method: 使用PEGASUS和PEGASUS-X模型家族，在一个中等规模的放射学报告公共数据集上进行微调，评估不同大小的检查点在相同训练数据上的表现，并使用词汇和语义指标监控模型性能。

Result: PEGASUS表现出与epoch-wise双下降或峰值-下降-恢复行为相关的不同阶段；对于PEGASUS-X，使用更大的检查点会导致性能下降。

Conclusion: 这项工作强调了在处理稀缺训练数据时微调高表达性模型的挑战和风险，为未来在专业领域开发更稳健的摘要模型微调策略奠定了基础。
...
Method: 使用双层SSL框架，将模型中间表示通过随机投影量化器离散化生成增强标签，同时使用原始输入的锚定标签稳定训练。采用一阶双层优化问题和可微分Gumbel-softmax选择实现端到端训练。

Result: 在多个数据集（960小时LibriSpeech、150小时AMI会议、5000小时YODAS）上验证，相比BEST-RQ获得一致性能提升，同时保持低复杂度和计算效率。

Conclusion: BiRQ消除了对外部标签编码器的需求，降低了内存成本，实现了端到端的迭代标签优化，在保持高效的同时显著提升了性能。
...
Method: PILOT框架分为两个阶段：第一阶段将自然语言人物描述转换为具有标准化分数的多维心理语言学配置文件；第二阶段使用这些配置文件沿可测量的变化轴引导生成过程。

Result: 在三个先进LLM上的评估显示，基于模式的方法显著减少了人工感的人物重复，同时提高了输出连贯性（轮廓分数从0.098提升到0.237，主题纯度从0.773提升到0.957）。

Conclusion: 研究发现基于模式的方法产生更简洁、主题一致性更高的输出，而自然语言方法提供更大的词汇多样性但可预测性降低。混合方法在保持输出多样性的同时保持了结构一致性。
...
Method: 使用英语(MUStARD++)和中文(MCSD 1.0)数据集，在零样本、少样本和LoRA微调设置下评估LLMs和多模态LLMs。除了直接分类外，还探索模型作为特征编码器，通过协作门控融合模块整合其表示。

Result: 实验结果显示，基于音频的模型在单模态中表现最强，而文本-音频和音频-视觉组合优于单模态和三模态模型。Qwen-Omni等MLLMs在零样本和微调设置下表现出竞争力。
...
<details>
  <summary>Details</summary>
Motivation: 随着多模态大语言模型在现实应用中的广泛部署，其在对抗条件下的安全性尚未得到充分探索，需要系统性评估。

Method: 使用26名红队成员生成的726个针对非法活动、虚假信息和不道德行为的提示，对四种模型进行测试，并由17名标注者对2,904个模型输出进行5级危害性评分。

Result: Pixtral 12B的有害响应率最高（约62%），Claude Sonnet 3.5最抗攻击（约10%）。意外的是，纯文本提示比多模态提示更容易绕过安全机制。

Conclusion: 研究结果强调了开发稳健的多模态安全基准的紧迫性，因为模型类型和输入模态都是危害性的显著预测因素。
...
Result: 在BAREC 2025共享任务中，严格赛道下句子级别测试集和盲测集QWK分别达到84.9%和85.7%，文档级别达到73.3%，QWK一致提升1-3个百分点。

Conclusion: 该方法通过不确定性感知解码有效减少高惩罚误分类，为阿拉伯语教育评估提供了结合统计保证和实际可用性的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 先前工作通常假设查询大小均匀，而异质查询大小引入了缓存选择的组合结构，使缓存替换过程在计算和统计上更具挑战性。

Method: 将最优缓存选择视为背包问题，采用基于累积的策略来有效平衡计算开销和缓存更新。

Result: 理论分析证明算法遗憾达到O(√MNT)界限，相比Berkeley的O(MN√T)结果改进了√MN系数。实验显示算法将总成本降低约12%。
...
Method: 构建评估框架，从三个核心方面比较在线俚语词典的人类俚语用法与GPT-4o和Llama-3生成的俚语：系统性偏见、创造力（新词创造和词汇重用）、信息量（作为模型蒸馏的黄金标准）

Result: 发现LLMs在感知俚语时存在显著偏见，虽然掌握了俚语的创造性方面知识，但这些知识与人类认知不够一致，无法支持语言分析等外推任务

Conclusion: LLMs在俚语理解方面仍有局限，其知识不足以可靠地用于语言分析等需要深度理解的任务
...
<details>
  <summary>Details</summary>
Motivation: 多语言指令微调对LLMs跨语言泛化至关重要，但高质量多语言训练数据的稀缺和相应构建方法的缺乏成为关键瓶颈。现有方法因依赖简单启发式或语言特定假设而难以跨语言泛化。

Method: 提出了M-DaQ方法，通过选择高质量和语义多样性的多语言IFT样本来改进LLMs的多语言能力，并首次系统研究了多语言环境下的表面对齐假设。

Result: 在18种语言上的实验结果表明，使用M-DaQ方法微调的模型相比基线实现了显著性能提升，胜率超过60%。人工评估进一步验证了这些增益，突显了响应中文化点的增加。
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的快速发展，AI生成文本与人类撰写文本之间的界限变得模糊，带来了错误信息、作者身份模糊和知识产权等社会风险，急需可靠的AI文本检测方法。

Method: 提出DNA-DetectLLM方法，采用DNA修复启发的方法，通过构建理想的AI生成序列，迭代修复非最优标记，并将累积修复努力量化为可解释的检测信号。

Result: 实验评估表明，该方法实现了最先进的检测性能，对多种对抗攻击和输入长度表现出强鲁棒性，在多个公共基准数据集上相对提升了5.55%的AUROC和2.08%的F1分数。

Conclusion: DNA-DetectLLM提供了一种直接且可解释的方法来捕捉AI生成文本与人类撰写文本之间的内在差异，为解决AI文本检测挑战提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 多语言LLMs面临训练语料中语言比例分配的关键挑战，由于复杂的跨语言交互和数据集规模敏感性，传统方法难以确定最优比例

Method: Climb框架引入跨语言交互感知的语言比例，量化语言间的相互依赖关系，采用两阶段优化：先平衡各语言的边际收益，再最大化分配向量的幅度

Result: 实验表明Climb能准确测量跨语言交互，使用Climb优化比例的LLMs在多语言任务上达到SOTA性能，甚至可与使用更多token的开源LLMs竞争

Conclusion: Climb为多语言数据分配提供了系统化解决方案，显著简化了复杂的多语言优化问题，为构建更强大的多语言LLMs提供了有效途径
...
<details>
  <summary>Details</summary>
Motivation: 现有基于相关性聚合的长上下文数据合成方法面临计算效率挑战，需要更高效的解决方案来训练能够处理长文档的大语言模型。
...
<details>
  <summary>Details</summary>
Motivation: 重新探讨语言在认知中的作用，研究语言是否仅仅是思想的表达工具，还是能够产生新思想的认知机制。通过AI和认知科学的最新发展来重新审视这一古老问题。

Method: 分析语言的两个核心特性：1）语言提供抽象概念的紧凑表示，便于推理；2）语言是集体智慧迭代输出的文化演化抽象。通过理论分析探讨这些特性如何支持领域通用智能的发展。

Result: 研究表明，语言学习实际上是在学习一个压缩的世界模型，能够逆向工程支持人类思维的概念和因果结构。无论是生物系统还是人工智能系统，接触语言都能获得这种压缩表示能力。

Conclusion: 语言可能是实现更通用AI系统和人类智能核心方面的关键。语言的双重特性使其成为连接个体认知与集体智慧的桥梁，为发展真正的人类级智能提供了重要启示。
...
<details>
  <summary>Details</summary>
Motivation: 现有检索增强生成系统存在检索相关性与生成效用之间的差距：检索到的文档可能在主题上相关，但缺乏生成过程中有效推理所需的内容。现有的"桥接"模块尝试重写检索文本以改善生成效果，但无法捕捉真实的文档效用。

Method: 提出R2U方法，关键区别在于通过过程监督直接优化生成正确答案的概率。由于直接监督成本高昂，还提出通过扩展LLMs监督来近似高效的蒸馏流程，帮助较小的重写模型更好地泛化。

Result: 在多个开放域问答基准测试中评估该方法，实证结果显示相对于强大的桥接基线方法有持续改进。
...
Method: 构建DivLogicEval基准，包含以反直觉方式组合的多样化自然语言语句；引入新的评估指标来减轻LLMs中偏差和随机性的影响。

Result: 通过实验证明了DivLogicEval中问题需要逻辑推理的程度，并比较了不同流行LLMs在逻辑推理方面的表现。

Conclusion: DivLogicEval提供了一个更可靠的大语言模型逻辑推理能力评估框架，有助于更准确地衡量模型的逻辑推理技能。
...
<details>
  <summary>Details</summary>
Motivation: 儿童通过社会互动高效学习语言，而大型语言模型通常仅通过大规模文本的下一词预测进行训练。这种对比激发了研究是否可以通过结合认知启发的高层次反馈来减少语言模型的训练数据需求。

Method: 训练学生模型生成故事，由教师模型对可读性、叙事连贯性和创造性进行评分。通过调整反馈循环前的预训练量，评估交互式学习对语言能力的影响。

Result: 高层次反馈具有极高的数据效率：在交互式学习中仅使用100万个单词的输入，叙事技能的提升效果相当于使用4.1亿个单词的下一词预测训练。

Conclusion: 交互式学习结合高层次反馈是训练语言模型的高效方法，能够显著减少数据需求，同时提升模型的叙事能力。
...
<details>
  <summary>Details</summary>
Motivation: 随着语音技术的快速发展，低延迟的语音人机通信需求日益增长。大多数自监督学习算法基于完整语音假设设计，而在流式应用中常见的部分语音片段需要妥协处理。因此需要一种统一的解决方案来同时支持流式和离线语音预训练。

Method: 提出基于分块的自监督学习算法，使用掩码预测损失优化，鼓励声学编码器利用同一分块和前一分块中未掩码的帧来恢复掩码语音帧的索引。采用复制和追加数据增强方法进行高效分块预训练，使用有限标量量化（FSQ）模块离散化输入语音特征，并采用分组掩码预测损失来缓解大码本带来的高内存和计算成本。

Result: 在Librispeech和Must-C数据集上的实验结果表明，该方法在语音识别和语音翻译任务中，无论是流式还是离线模式都能取得非常有竞争力的结果。
...
### [28] [Best-of-L: Cross-Lingual Reward Modeling for Mathematical Reasoning](https://arxiv.org/abs/2509.15811)
...
<details>
  <summary>Details</summary>
Motivation: 探究多语言LLMs中推理能力在不同语言间的变化情况，以及不同语言是否产生互补的推理路径。

Method: 训练一个奖励模型来对跨语言生成的回答进行排序，比较单语言和跨语言奖励模型的效果。

Result: 跨语言奖励模型相比单语言奖励模型显著提升了数学推理性能，跨语言采样在低采样预算下对英语尤其有益。
...
<details>
  <summary>Details</summary>
Motivation: 适应亿参数语言模型到下游任务仍然成本高昂，即使是参数高效微调(PEFT)也存在挑战

Method: 先进行短时预热微调，从预热模型和预训练模型的KL散度梯度中提取任务感知的引导向量，在解码过程中使用该向量引导输出分布

Result: 在三个任务和九个基准测试中，SVD配合四种标准PEFT方法将多项选择准确率提升高达5分，开放生成真实性提升2分，常识数据集也有1-2分提升

Conclusion: SVD为大型语言模型提供了轻量级、理论基础的更强任务适配路径
...
<details>
  <summary>Details</summary>
Motivation: 现有的科学信息提取主要依赖窄领域的实体关系提取，限制了跨学科研究的适用性，且难以捕捉科学信息的必要上下文，常导致碎片化或矛盾的陈述。

Method: 将科学信息提取定义为多阶段事件提取流程：(1) 将摘要分割为核心科学活动（背景、方法、结果、结论）；(2) 提取相应的触发词和细粒度参数。在500篇跨5个研究领域的摘要上进行人工标注。

Result: 实验显示，当前模型在社会科学和人文学科等领域表现不佳，存在性能差距。微调的事件提取模型、大语言模型和人工标注者之间存在差异。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于大语言模型的会议摘要系统容易产生幻觉、遗漏和不相关内容，需要提高摘要的准确性、忠实性和个性化能力。

Method: 1. FRAME管道：提取和评分关键事实，按主题组织，用于丰富摘要大纲
2. SCOPE协议：通过回答9个问题构建推理轨迹，实现个性化内容选择
3. P-MESA框架：多维度的无参考评估，判断摘要是否符合目标读者需求

Result: P-MESA达到≥89%的平衡准确率，与人类标注高度一致(r≥0.70)。FRAME将幻觉和遗漏减少了2/5个点，SCOPE在知识匹配和目标对齐方面优于仅提示的基线方法。

Conclusion: 研究主张重新思考摘要方法，通过语义重构和推理协议来提高控制性、忠实性和个性化程度。
...
<details>
  <summary>Details</summary>
Motivation: 现有机器遗忘方法需要明确的目标句子，不支持更广泛的概念（如人物或事件）遗忘，存在局限性。

Method: 提出新方法：提示LLM生成关于遗忘目标的知识三元组和解释性句子，对这些表示应用遗忘过程，使遗忘过程与LLM内部知识表示对齐。

Result: 在真实世界和合成数据集上的实验表明，该方法能有效实现概念级遗忘，同时保留无关知识。

Conclusion: 概念遗忘框架能够更精确和全面地移除概念，为LLM隐私和版权问题提供更有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的偏置项微调方法缺乏明确的指导原则来选择哪个偏置项进行微调，导致性能提升有限。本文旨在解决这一问题，通过理论分析提出一种有效的偏置项选择策略。

Method: 提出了一种偏置项选择方法，基于对偏置项在模型中的作用进行分析，选择对下游任务最关键的偏置项进行微调，形成偏置高效微调（BEFT）框架。

Result: 在多种大语言模型（参数规模从1.1亿到67亿）和下游任务（分类、多选、生成任务）上的实验表明，BEFT方法优于其他偏置选择方法，展现出更高的参数效率和性能。

Conclusion: BEFT方法通过智能选择偏置项进行微调，实现了参数高效性和性能的平衡，为参数高效微调技术提供了新的思路。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于抑制的遗忘方法（如梯度上升）只能控制模型输出，但无法消除模型内部嵌入的知识，且容易导致模型崩溃。需要一种能够真正实现知识遗忘的方法。

Method: 提出基于稀疏自编码器潜在空间的遗忘目标函数，将目标实体的内部激活从已知实体移向未知实体，使模型将目标实体识别为"未知"而非"已知"。

Result: 实验表明该方法能有效对齐遗忘目标的内部激活，在问答任务中显著降低目标知识的回忆率，同时对非目标知识影响较小。

Conclusion: 该方法通过直接干预内部激活实现了真正的遗忘，解决了传统抑制方法的局限性，为LLM隐私和版权保护提供了更有效的技术方案。
...
<details>
  <summary>Details</summary>
Motivation: 随着L2英语使用者数量的增长，对可靠的口语语言评估需求日益增加。现有方法存在级联管道错误传播或端到端模型仅处理短音频窗口而忽略语篇级证据的问题。

Method: 采用多目标学习与基于冻结Whisper ASR模型的语音先验进行声学感知校准，无需手工特征即可联合学习SLA的整体和特质级目标，连贯处理L2说话者的整个回答会话。

Result: 在Speak & Improve基准测试中，所提方法优于先前最先进的级联系统，并展现出强大的跨部分泛化能力，生成了一个适用于CALL应用的紧凑可部署评分器。

Conclusion: 该方法通过会话级单次处理和多模态基础模型，有效提升了口语语言评估的准确性和鲁棒性，为计算机辅助语言学习提供了更可靠的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 越南语作为低资源语言在医学机器翻译中研究不足，医疗英语-越南语翻译对于越南的医疗保健访问和沟通至关重要。

Method: 使用MedEV数据集，比较零样本、少样本和基于Meddict词典增强的提示策略，评估六种多语言LLM（0.5B-9B参数）。

Result: 模型规模是性能主要驱动因素，大模型在零样本设置下表现良好，少样本提示仅带来边际改进，而术语感知提示和嵌入检索能持续改善领域特定翻译。

Conclusion: 多语言LLM在医学英语-越南语翻译中展现出潜力但仍有局限性，需要术语增强策略来提升领域特定性能。
...
TL;DR: 提出了Think-Verbalize-Speak框架，将推理与语音输出解耦，通过中间verbalizing步骤将LLM的思考转化为适合语音的自然文本，在保持推理能力的同时提升语音自然度和简洁性。


<details>
  <summary>Details</summary>
Motivation: 现有方法直接将LLMs应用于语音对话系统会导致次优结果，因为文本推理与语音表达之间存在不匹配，且现有方法对推理性能的影响研究不足。

Method: 采用三阶段框架：思考-语言化-说话，核心是语言化步骤，将LLM的推理过程转化为适合语音输出的自然文本。还提出了ReVerT方法，基于增量异步摘要实现低延迟语言化。

Result: 在多个基准测试中，该方法显著提升了语音自然度和简洁性，同时对推理性能影响极小。

Conclusion: Think-Verbalize-Speak框架有效解决了LLMs在语音对话系统中的适配问题，在保持强大推理能力的同时优化了语音输出质量。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注SLMs对浅层声学和语音特征的编码，但对于其是否编码细微的句法和概念特征尚不清楚。本文借鉴大语言模型的语言能力评估方法，填补了这一研究空白。

Method: 采用最小对设计和诊断特征分析方法，对自监督学习（S3M）、自动语音识别（ASR）、语音压缩（codec）以及听觉大语言模型（AudioLLMs）编码器等多种SLMs进行分层和时间分辨分析。

Result: 研究发现所有语音模型对语法特征的编码都比概念特征更稳健，这是通过71个涵盖不同语言层次的任务分析得出的结论。

Conclusion: 本研究首次系统揭示了SLMs在句法和语义特征编码方面的能力差异，为理解语音模型的内部表征提供了重要见解。
...
<details>
  <summary>Details</summary>
Motivation: 在法学、医学等高风险领域评估长篇答案存在挑战，传统指标如BLEU和ROUGE无法捕捉语义正确性，现有LLM评估器往往将答案质量简化为单一分数。
...
<details>
  <summary>Details</summary>
Motivation: 构建支持语音的大型语言模型，探索更有效的跨模态对齐机制，为多语言和低资源语音LLM提供解决方案。

Method: 使用中间音频条件文本空间进行对齐，通过跨模态注意力机制在连续文本表示空间中融合Whisper的隐藏解码器状态和LLM的状态，支持离线和流式处理模式。
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在不同文化环境中部署，评估其文化理解能力变得至关重要。现有基准缺乏全面性，难以跨文化扩展和适应，因为它们缺乏成熟文化理论的指导且依赖专家手动标注。

Method: 受文化冰山理论启发，设计了一个包含3层140维的文化知识分类模式，指导自动构建针对任何语言和文化的特定知识库及相应评估数据集。

Result: 实验结果表明该方法能有效评估文化理解能力，同时发现现有大语言模型缺乏全面的文化能力，仅增加多语言数据并不一定能提升文化理解。

Conclusion: CultureScope是迄今为止最全面的文化理解评估框架，所有代码和数据文件已开源。
...
Conclusion: 研究结果突显了LMMs在APA中的潜力和局限性，指出了未来在细粒度建模和等级感知评估方面的工作方向。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在函数和文件级别的代码生成表现出色，但从零生成完整代码仓库仍然是一个基本挑战。自然语言由于其模糊性和冗长性，不适合准确表示复杂的软件结构。

Method: 引入Repository Planning Graph（RPG）作为持久化表示，统一规划和实现层面的能力、文件结构、数据流和函数。基于RPG开发ZeroRepo框架，包含三个阶段：提案级规划、实现级细化和图引导的代码生成与测试验证。

Result: 在RepoCraft基准测试中，ZeroRepo生成的仓库平均接近36K行代码，是最强基线（Claude Code）的3.9倍，其他基线的64倍。功能覆盖率达到81.5%，通过率为69.7%，分别比Claude Code高出27.3和35.8个百分点。

Conclusion: RPG能够建模复杂依赖关系，通过近线性缩放实现渐进式复杂规划，并增强LLM对仓库的理解，从而加速智能体定位。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的意见摘要公平性研究依赖于超参数调整或提供真实分布信息，但这些方法在实际应用中存在局限性：终端用户很少修改默认参数，且准确的分布信息通常难以获得。

Method: 借鉴认知科学研究，将频率框架提示（REFER）应用于LLM，通过明确参考类别和减少认知负荷来减少系统性偏见，与抽象概率表示相比，能更有效地引导语言模型处理信息。

Result: 实验结果表明，REFER能显著提升语言模型在意见摘要中的公平性，尤其在大型语言模型和使用更强推理指令时效果更为明显。

Conclusion: 频率框架提示是一种有效的实用方法，能够在不依赖超参数调整或真实分布信息的情况下，提升LLM意见摘要的公平性，为实际应用提供了可行解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在线性推理任务上表现出色，但在自然辩论等非线性结构推理方面研究不足，本文旨在探索LLMs是否能近似计算论证理论中的结构化推理。

Method: 使用定量论证辩论语义，基于两个NoDE数据集的对话格式辩论，在无图结构信息的情况下提示LLMs对论点进行排序，测试了多种LLMs和高级指令策略（包括思维链和上下文学习）。

Result: LLMs与QuAD排序显示出中等程度的一致性，但性能会随着输入长度增加或话语流中断而下降。高级提示策略有助于减轻与论点长度和位置相关的偏见。

Conclusion: 研究结果凸显了LLMs在建模形式论证语义方面的潜力和局限性，并推动了未来关于图感知推理的研究。
...
Result: 在多个长上下文任务实验中，UniGist显著提高了压缩质量，在细节回忆任务和长距离依赖建模方面表现尤其出色。
...
### [47] [UPRPRC: Unified Pipeline for Reproducing Parallel Resources -- Corpus from the United Nations](https://arxiv.org/abs/2509.15789)
*Qiuyang Lu,Fangjian Shen,Zhengkai Tang,Qiang Liu,Hexuan Cheng,Hui Liu,Wushao Wen*

Main category: cs.CL

TL;DR: 本文提出了一个完整的端到端解决方案，用于构建大规模多语言平行语料库，通过新的图辅助段落对齐算法，创建了超过7.13亿英文标记的公开可用语料库。


<details>
  <summary>Details</summary>
Motivation: 现有的基于联合国文档构建的多语言数据集存在过程不透明、难以复现和规模有限等问题，需要更好的解决方案来推进机器翻译研究。

Method: 采用完整的端到端流程，包括网络爬虫数据获取和文本对齐。核心是提出新的图辅助段落对齐（GAPA）算法，支持单机最小化示例和可选的分布式计算步骤。

Result: 构建的语料库包含超过7.13亿英文标记，规模是先前工作的两倍以上，是目前最大的完全由人工翻译、非AI生成内容的公开平行语料库。

Conclusion: 该研究提供了一个完全可复现的数据构建流程和高质量的大规模平行语料库，代码和语料库在MIT许可证下公开可用，有助于推动机器翻译领域的发展。
...
### [48] [RAVE: Retrieval and Scoring Aware Verifiable Claim Detection](https://arxiv.org/abs/2509.15793)
*Yufeng Li,Arkaitz Zubiaga*

Main category: cs.CL

TL;DR: RAVE框架结合证据检索与相关性、来源可信度的结构化信号，在可验证声明检测任务中优于仅基于文本和检索的基线方法。


<details>
  <summary>Details</summary>
Motivation: 社交媒体上错误信息的快速传播凸显了对可扩展事实核查工具的需求，而现有方法在处理模糊政治言论和多样化格式（如推文）时存在困难。

Method: 提出RAVE框架，结合证据检索与结构化信号（相关性和来源可信度）进行可验证声明检测。

Result: 在CT22-test和PoliClaim-test数据集上的实验表明，RAVE在准确率和F1分数上均优于文本和检索基线方法。

Conclusion: RAVE框架通过结合证据检索和结构化信号，有效提升了可验证声明检测的性能，为社交媒体事实核查提供了更可靠的解决方案。
...
TL;DR: 该研究探讨了视觉信息在训练中对音频和文本深度学习模型语言处理的影响，发现视觉基础能增强语音和书面语言表征的对齐，但主要改善的是词身份编码而非语义编码。


<details>
  <summary>Details</summary>
Motivation: 研究视觉基础如何影响深度学习模型中语言处理的内部表征，特别是比较音频和文本模型在视觉基础下的不同表现。

Method: 使用全局表征比较和针对性聚类分析，探究模型表征中的语音和语义可区分性。

Result: 视觉基础增强了语音和书面语言表征的对齐，但主要是通过改善词身份编码而非语义编码；语音表征在视觉基础下仍以语音特征为主，而文本表征的语义可区分性未得到改善。

Conclusion: 研究结果可为开发更有效的方法来丰富语音模型的视觉语义信息提供参考。
...
TL;DR: 本文介绍了Multi-Physics基准测试，用于评估多模态大语言模型在中文物理推理任务中的表现，包含5个难度级别和1,412个图像关联的多选题。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型评估基准在专业科学领域如物理方面存在显著不足，包括缺乏细粒度学科覆盖、忽视逐步推理过程、以及主要面向英语而未能系统评估视觉信息的作用。

Method: 采用双评估框架评估20个不同MLLM模型，分析最终答案准确性和逐步推理的完整性，并系统研究难度级别和视觉信息的影响。

Result: 提供了细粒度的社区资源，并开发了剖析最先进MLLM多模态推理过程的稳健方法。

Conclusion: Multi-Physics基准测试为中文物理推理提供了全面的评估工具，数据集和代码已开源。
...
TL;DR: 这篇综述论文探讨了超越传统事实核查的虚假信息检测方法，强调需要结合心理学概念（如认知偏见、社会动态和情绪反应）来构建更人性化的检测框架。


<details>
  <summary>Details</summary>
Motivation: 当前自动事实核查系统主要关注事实准确性，但虚假信息的危害超越了简单的虚假陈述，它利用了人们对信息的感知、解释和情绪反应方式，因此需要更人性化的检测方法。

Method: 通过分析最先进的虚假信息检测系统，结合人类心理学和行为学的视角，评估传统事实核查方法与心理学概念的相互作用。

Result: 揭示了当前方法的局限性，并确定了改进机会，提出了整合技术因素与人类认知和社会影响复杂性的神经行为模型等未来研究方向。

Conclusion: 结合心理学视角的虚假信息检测框架为更有效地检测和减轻虚假信息的社会危害提供了有前景的途径。
...
TL;DR: 本文提出使用保形预测方法为自动作文评分系统添加置信度评估，通过微调开源大语言模型并在三个数据集上验证，实现了90%风险水平下的可靠评分。


<details>
  <summary>Details</summary>
Motivation: 当前自动作文评分系统在公开基准上已达到接近人类的一致性，但由于缺乏置信度评估和解释性，在高风险考试中的实际应用仍然有限。

Method: 使用保形预测作为分布无关的包装器，为任何分类器提供集合值输出和正式覆盖保证。微调两个开源大语言模型（Llama-3 8B和Qwen-2.5 3B）在三个不同语料库上，并在90%风险水平下进行校准。

Result: 校准后的模型始终满足覆盖目标，同时保持预测集紧凑，表明开源中等规模大语言模型已能支持教师参与的自动作文评分。

Conclusion: 这是首个将保形预测和不确定性感知准确率结合用于作文评分的工作，讨论了扩展和更广泛用户研究作为未来方向。
...
<details>
  <summary>Details</summary>
Motivation: 为了在经典的softmax动态和hardmax动态之间找到平衡，允许对纯hardmax行为进行可控偏离，从而更灵活地建模注意力机制。

Method: 引入localmax动态模型，包含邻域影响参数和对齐敏感度参数，分析其收敛性和渐近行为，使用Lyapunov方法研究系统稳定性。

Result: 证明token状态的凸包收敛到凸多面体，但不能完全用最大对齐集描述，需要引入静止集来捕捉顶点附近token的不变行为。系统不会出现有限时间收敛。

Conclusion: localmax动态提供了介于softmax和hardmax之间的灵活注意力机制，揭示了在非对称设置下Lyapunov方法的局限性，为未来研究指明了方向。
...
TL;DR: DiEP提出了一种非均匀剪枝策略，通过自适应调整MoE模型各层的剪枝率，有效解决不同MoE层专家冗余度差异导致的性能下降问题。


<details>
  <summary>Details</summary>
Motivation: 现有MoE剪枝方法采用统一的剪枝稀疏度，但由于不同MoE层的专家冗余度存在差异，这种均匀剪枝会导致次优结果和性能下降。

Method: 将离散的全局搜索空间转化为连续空间，联合学习层间重要性，实现基于梯度的自适应剪枝，能够处理指数级增长的非均匀专家组合。

Result: 在五个先进MoE模型上的实验表明，DiEP在Mixtral 8×7B模型上仅保留一半专家就能达到原性能的92%，在MMLU数据集上比其他剪枝方法高出7.1%。

Conclusion: DiEP通过非均匀剪枝策略有效解决了MoE模型剪枝中的层间冗余度差异问题，显著提升了剪枝效果。
...
TL;DR: 该论文研究了大型语言模型在多轮对话中利用常识解决指代歧义的能力，发现现有模型在歧义持续时表现不佳，倾向于单一解释或覆盖所有可能引用，而非寻求澄清。简化语言提示会进一步削弱模型的表现，但通过直接偏好优化的微调可以显著改善歧义解决能力。


<details>
  <summary>Details</summary>
Motivation: 研究动机是系统性地探究大型语言模型是否能够利用常识在多轮对话中解决指代歧义，并分析模型在歧义持续时的行为表现，同时研究简化语言请求对这种能力的影响。

Method: 使用新颖的多语言评估数据集，通过LLM-as-Judge和人工标注测试了DeepSeek v3、GPT-4o、Qwen3-32B、GPT-4o-mini和Llama-3.1-8B等模型。对Llama-3.1-8B进行了直接偏好优化的微调实验。

Result: 研究发现当前LLM在有效解决歧义方面存在困难：模型倾向于承诺单一解释或覆盖所有可能引用，而不是采取对冲或寻求澄清的策略。简化提示显著减少了常识推理和多样化响应策略的使用。通过直接偏好优化的微调显著改善了所有请求类型的歧义解决能力。

Conclusion: 研究结果强调了需要先进的微调技术来改进LLM处理歧义的能力，并确保在不同沟通风格下的稳健性能。
...
Conclusion: CodeRAG通过系统性的改进有效解决了仓库级代码补全中的关键问题，为检索增强的代码补全提供了有效解决方案。
...
TL;DR: MICA是一个面向工业工作流程的多代理协调助手系统，通过语音交互提供实时指导，具备感知能力和隐私保护特性，可在离线硬件上部署。
...
Method: MICA协调五个角色专业化的语言代理，采用自适应步骤融合（ASF）技术动态混合专家推理与自然语音反馈的在线适应，并通过安全检查器进行审计。

Result: 实验表明MICA在任务成功率、可靠性和响应性方面持续优于基线结构，同时可在实际离线硬件上部署。

Conclusion: MICA是实现可部署、隐私保护的多代理助手系统的重要一步，适用于动态工厂环境。
...
<details>
  <summary>Details</summary>
Motivation: 填补神经算法推理在背包问题这一连接经典算法与组合优化的伪多项式问题上的空白，标准NAR基准中缺少对该问题的研究。

Method: 设计了两阶段神经算法推理器：第一阶段构建动态规划表，第二阶段从表中重构解决方案，通过动态规划监督对中间状态进行建模。
...
<details>
  <summary>Details</summary>
Motivation: 随着机器学习和人工智能在智能交通网络中的应用增加，强化学习被证明是一种有前景的方法。然而，在交通信号控制中使用强化学习时，由于输入数据分布相对于训练数据分布的动态变化，训练出的RL代理的可靠性存在问题，这可能带来不良甚至有害的后果。

Method: 本文评估和分析了一种名为MetaLight的最先进的元强化学习方法，该方法旨在解决输入数据分布变化的问题。

Result: 研究发现，在某些条件下MetaLight确实能产生相当好的结果，但在其他条件下可能表现不佳，误差高达22%。

Conclusion: 元强化学习方案通常不够鲁棒，甚至可能带来重大可靠性问题，需要进一步改进以确保在实际交通信号控制中的可靠性。
...
Result: 评估了三种嵌入模型，系统在无启发式反馈或真实相关性标签情况下仍显示出潜力
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型基于预训练知识进行推理的能力虽然有用，但也可能导致知识驱动幻觉，即模型输出与提供的明确证据相矛盾，这在证据导向的领域存在严重可靠性问题。

Method: 在业务流程管理领域进行受控实验，设计标准流程和故意非典型流程的输入场景，测量LLM对提供证据的忠实度，检验模型是否会被预训练知识覆盖明确证据。

Result: 实验证实了知识驱动幻觉现象的存在，LLM在处理与预训练知识冲突的证据时，倾向于依赖内部知识而非提供的明确证据。

Conclusion: 研究提出了评估AI生成内容可靠性的方法学，强调在证据导向领域需要对AI生成物进行严格验证的重要性。
...
TL;DR: 本文提出了一个诊断框架，用于评估和促进专家行为向LLM驱动智能体的转移。该框架整合了黄金数据集、银数据集和基于LLM的智能体评判器，能够发现认知失败并引导智能体达到专家级推理水平。
...
Method: 框架包含三个核心组件：(i)专家注释的黄金数据集，(ii)通过受控行为突变生成的银数据集，(iii)基于LLM的智能体评判器，可评分并制定针对性改进方案。改进建议嵌入向量化推荐图中，实现跨系统实例的可重用改进轨迹。

Result: 在多智能体招聘助理系统上的实验表明，该框架能够发现潜在的认知失败（如偏见措辞、提取漂移和工具误路由），同时引导智能体达到专家级推理和风格。
...
TL;DR: FragmentRetro是一种新型的逆合成方法，通过碎片化算法、库存感知探索和指纹筛选实现二次复杂度，相比传统树搜索的指数复杂度具有显著计算优势。


<details>
  <summary>Details</summary>
Motivation: 传统树搜索方法在计算机辅助合成规划中存在指数级计算复杂度问题，限制了其在大分子合成中的应用。

Method: 结合BRICS和r-BRICS碎片化算法，采用库存感知探索和模式指纹筛选，递归组合分子碎片并验证其在构建块集中的存在性。

Result: 在PaRoutes、USPTO-190和天然产物测试中，FragmentRetro实现了高解决率，计算复杂度为O(h²)，远优于树搜索的O(b^h)和DirectMultiStep的O(h^6)。

Conclusion: FragmentRetro虽然专注于识别基于碎片的解决方案而非完整反应路径，但其计算优势和生成战略起始候选物的能力使其成为可扩展自动化合成规划的有力基础组件。
...
<details>
  <summary>Details</summary>
Motivation: 解决微服务系统中故障根因定位的挑战，利用大语言模型的多模态理解和逻辑推理能力来提升故障分析的准确性和效率。

Method: 1. 结合Drain日志解析算法和多级数据过滤机制压缩海量日志；2. 采用隔离森林无监督学习与状态码验证的双异常检测方法；3. 设计统计对称比过滤机制和两阶段LLM分析策略实现全栈现象总结；4. 使用跨模态提示深度整合多模态异常信息。

Result: 系统在复杂微服务故障场景中表现出优越性能，最终得分为50.71。消融研究验证了各模态数据的互补价值和系统架构的有效性。
...
TL;DR: 本文提出了一个解决C++编译错误的自动化修复框架，包括CCrepair数据集、基于强化学习的修复方法，以及LLM作为评判者的评估系统。


<details>
  <summary>Details</summary>
Motivation: 解决C++编译错误修复面临的两个主要挑战：缺乏大规模高质量数据集，以及传统监督方法难以生成语义正确的补丁。

Method: 采用三阶段方法：1) 构建CCrepair数据集；2) 提出基于混合奖励信号的强化学习范式；3) 建立两阶段评估系统，使用LLM作为评判者。

Result: RL训练的Qwen2.5-1.5B-Instruct模型性能与Qwen2.5-14B-Instruct模型相当，验证了训练范式的效率。

Conclusion: 该工作为研究社区提供了有价值的新数据集和更有效的训练评估范式，为更实用可靠的自动化编程助手铺平了道路。
...
<details>
  <summary>Details</summary>
Motivation: RPA虽然能有效自动化基于规则的结构化任务，但其符号性质在处理更复杂任务时存在固有局限性。机器学习概念为智能RPA提供了扩展可自动化任务范围的机会。
...
<details>
  <summary>Details</summary>
Motivation: 支持研究人员映射与生理系统相关的数据，特别是研究器官的相关性，神经系统在协调和传递全身信号中起关键作用

Method: 创建包含知识表示模型和知识管理工具套件的框架，KR模型便于生理学专家捕捉解剖实体间的相互作用，KM工具帮助建模者将高层次抽象转换为详细的生理过程模型

Result: 建立了能够整合外部本体和知识图谱的生理回路图表示框架

Conclusion: ApiNATOMY为多尺度生理回路图的表示和管理提供了有效的基础设施支持
...
TL;DR: CLIMB是一个自动化构建职业分类系统的框架，通过全局语义聚类和多智能体系统从原始招聘数据中创建高质量、数据驱动的分类法。


<details>
  <summary>Details</summary>
Motivation: 现有职业分类方法存在不足：人工构建速度慢，自动化方法要么无法适应动态区域市场（自上而下），要么难以从噪声数据构建连贯的层次结构（自下而上）。

Method: CLIMB使用全局语义聚类提炼核心职业，然后采用基于反射的多智能体系统迭代构建连贯的层次结构。

Result: 在三个不同的真实数据集上，CLIMB生成的分类法比现有方法更连贯和可扩展，并成功捕捉了独特的区域特征。

Conclusion: CLIMB框架能够完全自动化地从原始招聘信息中创建高质量、数据驱动的职业分类法，解决了现有方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 工业4.0环境下监控系统正从传统规则架构转向数据驱动方法，需要系统比较两种方法并探索融合方案。

Method: 通过分析两种方法各自的优势、局限性和应用场景，建立基本评估框架来比较关键特性。

Result: 规则系统在稳定环境中具有高可解释性和确定性，但适应性差；数据驱动系统能检测隐藏异常但面临数据可用性和可解释性挑战。

Conclusion: 未来工业监控应采用混合解决方案，结合规则逻辑的透明性和机器学习的分析能力，实现更智能、灵活的工业环境。
...
TL;DR: 该研究评估了通过Model Context Protocol（MCP）将大型语言模型（LLM）连接到医院电子健康记录（EHR）数据库的可行性，在真实医院环境中测试LLM自主检索临床相关信息的能力。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM在医学领域显示出潜力，但由于对EHR系统的访问受限，其在医院中的部署受到限制。MCP技术能够实现LLM与外部工具的集成，为医院AI应用提供可能。

Method: 开发了EHR-MCP框架，将自定义MCP工具与医院EHR数据库集成，使用GPT-4.1通过LangGraph ReAct代理进行交互。测试了6个感染控制团队相关任务，回顾性分析了8名患者，与医生生成的金标准进行比较。

Result: LLM能够一致选择和执行正确的MCP工具，除两个任务外，其他任务都达到接近完美的准确率。在需要时间相关计算的复杂任务中表现较差，错误主要来自参数不正确或工具结果误解。
...
Result: 在视觉和NLP领域的实验表明，ASAC能提高分类准确性、加速学习过程，并展现出对噪声数据、分布外数据的鲁棒性，在多任务设置中也有改进表现。

Conclusion: ASAC成功建立了认知科学与机器学习之间的联系，为AI系统中注意力机制的高效利用提供了新思路，在对抗攻击鲁棒性、学习效率和迁移学习等方面展现出潜力。
